...
def main() -> None:
    ...
    if outpath:
        new_hash = hashlib.sha256(output.encode('utf-8')).hexdigest()
        ...
        try:
            ...
            m = re.match(r'^//\s*hash:\s*([0-9a-fA-F]+)\s*$', first_line)
            if m:
        ...
        if existing_hash == new_hash:
            print(f'Output up to date, skipping write to {outpath}', file=sys.stderr)
