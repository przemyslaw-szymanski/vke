...
namespace VKE
{
    namespace RenderSystem
    {
        ...
        Result CContextBase::_CreateExecuteBatch( uint32_t batchBufferIndex, uint32_t batchIndex, SExecuteBatch* pOut )
        {
        ...
        SExecuteBatch* CContextBase::_AcquireExecuteBatch()
        {
            ...
            VKE_LOG( m_pName << ": acquire batch: " << m_pCurrentExecuteBatch
                             << ", gpu fence: " << m_pCurrentExecuteBatch->hSignalGPUFence );
            VKE_ASSERT( !m_pDeviceCtx->_IsGPUFenceLocked( m_pCurrentExecuteBatch->hSignalGPUFence ) );
