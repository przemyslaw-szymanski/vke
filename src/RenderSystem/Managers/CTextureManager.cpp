...
namespace VKE
{
    namespace RenderSystem
    {
        ...
        TEXTURE_ASPECT ConvertTextureUsageToAspect( const TEXTURE_USAGE& usage )
        {
            ...
            if( usage & TextureUsages::COLOR_RENDER_TARGET || usage & TextureUsages::FILE_IO )
            {
        ...
        CTextureManager::CTextureManager( CDeviceContext* pCtx ) : m_pDevice{ pCtx }
        {
        }
        ...
        void CTextureManager::Destroy()
        {
            for( auto& Pair: m_Samplers.Container )
            {
                ...
                {
                    m_pDevice->NativeAPI().DestroySampler( &pCurr->m_hDDIObject );
                }
            }
            ...
            for( uint32_t i = 1; i < m_TextureViews.vPool.GetCount(); ++i )
            {
                ...
                {
                    m_pDevice->NativeAPI().DestroyTextureView( &pCurr->m_hDDIObject );
                }
            }
...
#endif
                    hApiObj = m_pDevice->_NativeAPI().CreateTexture( Desc );
                        ...
                        Core::CreateResourceFlags::ASYNC )
                    {
                        if( !pTex->IsResourcePending() )
                        {
                            ...
                            ret = m_pDevice->GetRenderSystem()->GetEngine()->GetThreadPool()->AddTask(
                                ...
                                LoadTextureTask.GetDebugText(),
                                LoadTextureTaskFunc,
                                pTex,
                                Info.FileInfo.FileName,
                                Info.CreateInfo.flags );
                        }
                    }
                    else
                    {
        ...
        Result CTextureManager::_CreateAPIObject( const Core::ImageHandle& hImg, StagingBufferFlags updateInfoFlags,
                                                  CTexture** ppInOut )
        {
            VKE_ASSERT( ppInOut != nullptr && *ppInOut != nullptr );
        ...
        CTexture* CTextureManager::_CreateTextureTask( const STextureDesc& Desc )
        {
            ...
            VKE_ASSERT2( ( Desc.memoryUsage & MemoryUsages::TEXTURE ) == MemoryUsages::TEXTURE,
                         "MemoryUsages::TEXTURE bit must be set in memoryUsage flags" );
            ...
            }
            {
                ...
                if( pTex != nullptr )
                {
                    pTex->Init( Desc );
                    {
                        ...
                        {
                            pTex->m_hDDIObject = m_pDevice->_NativeAPI().CreateTexture( Desc );
        ...
        Result CTextureManager::_UploadTextureMemoryTask( StagingBufferFlags Flags, Core::CImage* pImg,
                                                          CTexture** ppInOut )
        {
            Result            ret;
            CTexture*         pTex = *ppInOut;
        ...
        Result CTextureManager::_UploadTextureMemoryTask( const SUpdateMemoryInfo& Info, CTexture** ppInOut )
        {
            ...
            {
                VKE_LOG_ERR( "Upload texture: '" << pTex->GetDesc().GetDebugName()
                                                 << "' memory fail due to texture fence in use." );
            }
        ...
        TextureViewHandle CTextureManager::CreateTextureView( const STextureViewDesc& Desc )
        {
            ...
            if( pView != nullptr )
            {
                ...
                pView->Init( Desc, pTex );
                {
                    ...
                    {
                        pView->m_hDDIObject = m_pDevice->_NativeAPI().CreateTextureView( Desc );
                    }
                    if( pView->m_hDDIObject != NativeAPI::Null )
                    {
                        hRet.handle      = handle;
        ...
        void CTextureManager::_DestroyTexture( CTexture** ppInOut )
        {
            ...
            {
                m_pDevice->_NativeAPI().DestroyTexture( &pTex->m_hDDIObject );
            }
            Memory::DestroyObject( &m_TexMemMgr, &pTex );
            *ppInOut = nullptr;
        }
        ...
        void CTextureManager::_DestroyTextureView( CTextureView** ppInOut )
        {
            ...
            m_pDevice->_NativeAPI().DestroyTextureView( &pView->m_hDDIObject );
            Memory::DestroyObject( &m_TexViewMemMgr, &pView );
            *ppInOut = nullptr;
        }
        ...
        SamplerHandle CTextureManager::CreateSampler( const SSamplerDesc& Desc )
        {
            ...
            if( pSampler )
            {
                ...
                {
                    pSampler->Init( Desc );
                    pSampler->m_hDDIObject = m_pDevice->NativeAPI().CreateSampler( pSampler->m_Desc );
        ...
        void CTextureManager::_DestroySampler( CSampler** ppInOut )
        {
            ...
            m_pDevice->NativeAPI().DestroySampler( &pSampler->m_hDDIObject );
            pSampler->_Destroy();
