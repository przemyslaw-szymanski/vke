...
#error "implement"
        ...
        static const SImageExtValue g_aFileExtensions[ ImageFileFormats::_MAX_COUNT ] = {
            { "", ImageFileFormats::UNKNOWN },  { "bmp", ImageFileFormats::BMP }, { "dds", ImageFileFormats::DDS },
            { "gif", ImageFileFormats::GIF },   { "hdr", ImageFileFormats::HDR }, { "jpg", ImageFileFormats::JPG },
            { "jpeg", ImageFileFormats::JPG },  { "png", ImageFileFormats::PNG }, { "tif", ImageFileFormats::TIFF },
            { "tiff", ImageFileFormats::TIFF }, { "tga", ImageFileFormats::TGA },
        };
        ...
        vke_force_inline DirectX::WICCodecs MapFileFormatToWICCodec( IMAGE_FILE_FORMAT fmt )
        {
            static const DirectX::WICCodecs aCodecs[] = {
                ...
                DirectX::WIC_CODEC_JPEG, DirectX::WIC_CODEC_PNG, DirectX::WIC_CODEC_TIFF,
        ...
        Result CImageManager::_CreateDirectXTexImage( const CFile* pFile, CImage** ppInOut )
        {
            ...
            {
                static const DirectX::DDS_FLAGS ddsFlags =
                    DirectX::DDS_FLAGS_ALLOW_LARGE_FILES | DirectX::DDS_FLAGS_LEGACY_DWORD;
            ...
            else // Copied from Texconv.cpp
            {
                ...
                static_assert( static_cast< int >( DirectX::WIC_FLAGS_DITHER_DIFFUSION ) ==
                                   static_cast< int >( DirectX::TEX_FILTER_DITHER_DIFFUSION ),
                               "WIC_FLAGS_* & TEX_FILTER_* should match" );
                static_assert( static_cast< int >( DirectX::WIC_FLAGS_FILTER_POINT ) ==
                                   static_cast< int >( DirectX::TEX_FILTER_POINT ),
                               "WIC_FLAGS_* & TEX_FILTER_* should match" );
                static_assert( static_cast< int >( DirectX::WIC_FLAGS_FILTER_LINEAR ) ==
                                   static_cast< int >( DirectX::TEX_FILTER_LINEAR ),
                               "WIC_FLAGS_* & TEX_FILTER_* should match" );
                static_assert( static_cast< int >( DirectX::WIC_FLAGS_FILTER_CUBIC ) ==
                                   static_cast< int >( DirectX::TEX_FILTER_CUBIC ),
                               "WIC_FLAGS_* & TEX_FILTER_* should match" );
                static_assert( static_cast< int >( DirectX::WIC_FLAGS_FILTER_FANT ) ==
                                   static_cast< int >( DirectX::TEX_FILTER_FANT ),
                               "WIC_FLAGS_* & TEX_FILTER_* should match" );
        ...
        CImage* CImageManager::_Copy( const SCreateCopyImageInfo& Info )
        {
            ...
            if( pSrcImg!= nullptr )
            {
                ...
                Utils::SHash Hash;
                Hash.Combine( srcHash, Region.Size.width, Region.Size.height, Region.Offset.x, Region.Offset.y );
                ...
                if( res == VKE_OK )
                {
                    ...
                    if( hr == S_OK )
                    {
                        ...
                        if( needDecompress )
                        {
                            hr = DirectX::Decompress( pSrcDx->GetImages(),
                                                      pSrcDx->GetImageCount(),
                                                      *pSrcMeta,
                                                      DXGI_FORMAT_UNKNOWN,
                                                      DecompressedImage );
                            if( hr == S_OK )
                            {
                                ...
                                hr = TmpDstImg.Initialize2D(
                                    pSrcMeta->format, Rect.w, Rect.h, pSrcMeta->arraySize, pSrcMeta->mipLevels );
                                if( hr == S_OK )
                                {
                                    pDstDx = &TmpDstImg;
                                }
                        ...
                        {
                            hr = pDstImg->m_DXImage.Initialize2D(
                        ...
                        if( hr == S_OK )
                        {
                            ...
                            hr = DirectX::CopyRectangle( *pSrcDx->GetImage( 0, 0, 0 ),
                                                         Rect,
                            ...
                            if( hr == S_OK )
                            {
                                ...
                                {
                                    hr = DirectX::Compress( pDstDx->GetImages(),
                                                            pDstDx->GetImageCount(),
                                                            SrcMetadata,
                                                            SrcMetadata.format,
...
#error "implement"
        ...
        Result CImageManager::_SliceCompressed( CImage* pImg, const SSliceImageInfo& Info, ImageHandle* pOut )
        {
            Result      ret  = VKE_FAIL;
            ImageHandle hRet = INVALID_HANDLE;
#if VKE_USE_DIRECTXTEX
            ...
            ::HRESULT             hr = DirectX::Decompress(
                pSrcDx->GetImages(), pSrcDx->GetImageCount(), SrcMeta, DXGI_FORMAT_UNKNOWN, DecompressedImg );
            if( hr == S_OK )
            {
                ...
                for( uint32_t i = 0; i < Info.vRegions.GetCount(); ++i )
                {
                    ...
                    if( hr == S_OK )
                    {
                        ...
                        hr = DirectX::CopyRectangle( *DecompressedImg.GetImage( 0, 0, 0 ),
                                                     Rect,
                        ...
                        if( hr == S_OK )
                        {
                            Utils::SHash Hash;
                            Hash.Combine(
                                srcHash, Region.Size.width, Region.Size.height, Region.Offset.x, Region.Offset.y, i );
                            CImage* pDstImg;
                                ...
                                if( hr == S_OK )
                                {
                                    ...
                                    if( hr == S_OK )
                                    {
                                        ...
                                        if( Info.pSavePath )
                                        {
                                            ...
                                            Platform::File::GetDirectory(
                                                Info.pSavePath, (uint32_t)strlen( Info.pSavePath ), &pDir );
...
#error "implement"
        ...
        ImageHandle CImageManager::CreateNormalMap( const ImageHandle& hSrcImg )
        {
            ...
            if( VKE_SUCCEEDED( res ) )
            {
                ...
                ::HRESULT hr       = DstImage.Initialize2D(
                    DXGI_FORMAT_R8G8B8A8_UNORM, Meta.width, Meta.height, Meta.arraySize, Meta.mipLevels );
                if( hr == S_OK )
                {
                    hr = DirectX::ComputeNormalMap( pImg->m_DXImage.GetImages(),
                                                    pImg->m_DXImage.GetImageCount(),
                                                    Meta,
                                                    DirectX::CNMAP_DEFAULT,
                                                    0.0f,
        ...
        Result CImageManager::Save( const SSaveImageInfo& Info )
        {
            ...
            if( codec != 0 )
            {
                ...
                ::HRESULT hr   = DirectX::SaveToWICFile( Img, DirectX::WIC_FLAGS_NONE, guid, Name.GetData(), nullptr );
...
#error "implement"
        ...
        Result CImageManager::_Resize( const ImageSize& NewSize, CImage** ppInOut )
        {
            ...
            ::HRESULT             hr =
                NewImage.Initialize2D( Meta.format, NewSize.width, NewSize.height, Meta.arraySize, Meta.mipLevels );
            ...
            if( hr == S_OK )
            {
                ...
                if( needDecompress )
                {
                    hr = DirectX::Decompress( pImg->m_DXImage.GetImages(),
                                              pImg->m_DXImage.GetImageCount(),
                                              Meta,
                                              DXGI_FORMAT_UNKNOWN,
                                              TmpImg );
                ...
                hr = DirectX::Resize( pSrcImg->GetImages(),
                                      pSrcImg->GetImageCount(),
                                      *pSrcMetadata,
                                      (size_t)NewSize.width,
                                      (size_t)NewSize.height,
                                      DirectX::TEX_FILTER_DEFAULT,
                                      NewImage );
                if( hr == S_OK )
                {
                    ...
                    if( needDecompress )
                    {
                        ...
                        hr = DirectX::Compress( NewImage.GetImages(),
                                                NewImage.GetImageCount(),
                                                NewImage.GetMetadata(),
                                                Meta.format,
