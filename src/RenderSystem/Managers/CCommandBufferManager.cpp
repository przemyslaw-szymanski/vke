...
namespace VKE
{
    ...
    {
        CCommandBufferManager::CCommandBufferManager( CContextBase* pCtx ) : m_pCtx( pCtx )
        {
        }
        ...
        Result CCommandBufferManager::_CreateCommandBuffers( uint32_t count, SCommandPool* pPool,
                                                             CCommandBuffer** ppOut )
        {
            assert( pPool );
            auto&  vFreeCbs = pPool->vpFreeCommandBuffers;
            Result ret      = VKE_OK;
            ...
            if( ppOut != nullptr )
            {
                for( uint32_t i = 0; i < count; ++i )
                {
                    ...
                    VKE_ASSERT2( ( *ppOut )->m_pBaseCtx == m_pCtx,
                                 "Command buffer must be created for the same context" );
                }
            }
            return ret;
        }
