...
namespace VKE::RenderSystem::D3D12
{
    ...
    template< typename T >
    vke_force_inline auto ToNativeArray( const T* pObj )
    {
        using NativeT = decltype( ToNative( std::declval< T >() ) );
        return reinterpret_cast< const NativeT* >( pObj );
    }

    template< typename T >
    vke_force_inline auto ToNativeArray( T* pObj )
    {
        using NativeT = decltype( ToNative( std::declval< T >() ) );
        return reinterpret_cast< NativeT* >( pObj );
    }

    template< typename EngineT, typename NativeT >
    vke_force_inline EngineT FromNative( NativeT v )
    {
...
#define UNIMPLEMENTED_D3D12_METHOD()                                                                                   \
    ...
    SImplementation::SDescriptorHeapInfo* SImplementation::GetDescriptorHeap( const NativeAPI::Device&    pDevice,
                                                                              ...
                                                                              D3D12_DESCRIPTOR_HEAP_FLAGS Flags )
    {
        SImplementation::SDescriptorHeapInfo* pDescriptorHeap = nullptr;
    ...
    void SImplementation::ReleaseDescriptorHeaps()
    {
        for( uint32_t i = 0; i < m_vDescriptorHeapPool.GetCount(); i++ )
        {
            m_vDescriptorHeapPool[ i ].pDescriptorHeap->Release();
            m_vDescriptorHeapPool[ i ].pDescriptorHeap = NativeAPI::Null;
        }

        m_vDescriptorHeapPool.Clear();
    }
...
#define D3D12_RHI_LOG( _msg )
    ...
    VKE::Result NativeAPI::CustomTypes::SFence::Wait( UINT64 value, DWORD timeout )
    {
        VKE_ASSERT( pObject != nullptr );
        ...
        HRESULT hr = pObject->SetEventOnCompletion( value, hEvent );
        ...
        DWORD waitResult = ::WaitForSingleObjectEx( hEvent, timeout, FALSE );
    ...
    void NativeAPI::CustomTypes::SFence::Wait( NativeAPI::D3D12CommandQueue* pQueue, UINT64 value )
    {
        VKE_ASSERT( pObject != nullptr );
    ...
    namespace Map
    {
        ...
        D3D_PRIMITIVE_TOPOLOGY GetPrimitiveTopology( PRIMITIVE_TOPOLOGY EngineTopology )
        {
            static const D3D_PRIMITIVE_TOPOLOGY ascNativeMap[] = {
            ...
            static_assert( PRIMITIVE_TOPOLOGY::_MAX_COUNT == _countof( ascNativeMap ) );
            return ascNativeMap[ static_cast< size_t >( EngineTopology ) ];
        }
    ...
    namespace Convert
    {
        ...
        vke_force_inline D3D12_BARRIER_SUBRESOURCE_RANGE GetSubresourceIndex( RHI::Texture              hTexture,
        ...
        void GetResourceDesc( const STextureDesc& EngineDesc, const SImplementation::SDeviceFeatures& NativeFeatures,
        ...
        NativeAPI::D3D12ResourceDesc GetResourceDesc( const SBufferDesc&                      EngineDesc,
                                                      const SImplementation::SDeviceFeatures& NativeFeatures )
        {
    ...
    CD3D12API::~CD3D12API()
    {
        ...
        {
            SImplementation::spFactory->Release();
        ...
        if( SImplementation::sDebugLayerEnabled )
        {
            ...
            if( SUCCEEDED( DXGIGetDebugInterface1( 0, IID_PPV_ARGS( &pDebug ) ) ) )
            {
                pDebug->ReportLiveObjects(
                    DXGI_DEBUG_ALL, DXGI_DEBUG_RLO_FLAGS( DXGI_DEBUG_RLO_ALL | DXGI_DEBUG_RLO_IGNORE_INTERNAL ) );
            }
    ...
    Result CD3D12API::QueryAdaptersImpl( AdapterInfoArray* pOut )
    {
        ...
        while( Index < MAX_ADAPTERS )
        {
            ...
            // pAdapter keeps its own reference, so release the temporary here to avoid a DXGI leak.
            pAdapter1->Release();
            pAdapter1 = NativeAPI::Null;
    ...
    Result CD3D12API::LoadImpl( const SDDILoadInfo& Info, SDriverInfo* pOut )
    {
        ...
        SImplementation::SDeviceFeatures::sTearingSupported = ( AllowTearing == TRUE );
    ...
    Result CD3D12API::CreateDeviceImpl( const SCreateDeviceDesc& Info, CDeviceContext* pCtx )
    {
        ...
        HRESULT Result = D3D12CreateDevice( m_pImplementation->m_hAdapter,
        ...
        FeatureSupport.Init( m_pImplementation->m_hDevice );

        auto& Features = m_pImplementation->Features;
        ...
        {
            m_pImplementation->Properties.Memory.DescriptorHeapSizes[ i ] =
                m_pImplementation->m_hDevice->GetDescriptorHandleIncrementSize(
    ...
    void CD3D12API::DestroyDeviceImpl()
    {
        ...
        {
            for( auto& hQueue: QueueFamily.vQueues )
            {
                auto pNativeQueue = ToNative( hQueue );
                if( pNativeQueue != NativeAPI::Null )
                {
                    pNativeQueue->Release();
                }
                hQueue = RHI::Null;
            }
            QueueFamily.vQueues.Clear();
        }

        m_pImplementation->ReleaseDescriptorHeaps();

        if( m_pImplementation->m_hDevice != NativeAPI::Null )
        {
            m_pImplementation->m_hDevice->Release();
                                   ...
                                   bool Required = false )
    {
        ...
        NativeDesc.Type     = NativeType;
        NativeDesc.Priority = D3D12_COMMAND_QUEUE_PRIORITY_HIGH;
        NativeDesc.Flags    = D3D12_COMMAND_QUEUE_FLAG_NONE;
        NativeDesc.NodeMask = Helper::GetNodeMask();
    ...
    void CD3D12API::QueryDeviceInfoImpl( SDeviceInfo* pOut )
    {
        ...
        auto& l1Budget = m_pImplementation->Properties.Memory.localBudget;
        auto& l0Budget = m_pImplementation->Properties.Memory.hostBudget;

        // Right now just assume budgets for heaps for each level.
        auto& Heaps                            = m_pImplementation->Properties.Memory.HeapProperties;
        ...
        for( int i = 0; i < QueueTypes::_MAX_COUNT; i++ )
        {
            ...
            {
                pQueue =
                    CreateCommandQueue( m_pImplementation->m_hDevice, Type, Type == D3D12_COMMAND_LIST_TYPE_DIRECT );
            }
    ...
    void CD3D12API::DestroyTextureViewImpl( RHI::TextureView* pInOut, const void* pAllocator )
    {
        ...
        {
            Memory::DestroyObject( &HeapAllocator, &pNativeTextureView );
            *pInOut = RHI::Null;
        }
    }
                                                     ...
                                                     const void*            pAllocator )
    {
        ...
        {
            NativeAPI::CustomTypes::SRenderPass::SSubpass DefaultSubpass;
            pNativeRenderPass->vSubpasses.PushBack( DefaultSubpass );
            pDefaultSubpass = &pNativeRenderPass->vSubpasses.Back();
        }
        ...
        auto pDescriptorHeapRTV = m_pImplementation->GetDescriptorHeap(
            m_pImplementation->m_hDevice, D3D12_DESCRIPTOR_HEAP_TYPE_RTV, D3D12_DESCRIPTOR_HEAP_FLAG_NONE );
        ...
        for( uint32_t index = 0; index < EngineRenderPassDesc.vRenderTargets.GetCount(); index++ )
        {
            ...
            const auto& NativeResourceView = *ToNative( EngineRenderTarget.hRHIView );
            ...
            else if( isDepthStencilView )
            {
                ...
                m_pImplementation->m_hDevice->CreateDepthStencilView(
                    NativeResourceView.pResource,
    ...
    void CD3D12API::DestroyRenderPassImpl( RHI::RenderPass* pInOut, const void* pAllocator )
    {
        ...
        {
            Memory::DestroyObject( &HeapAllocator, &pNativeRenderPass );
            *pInOut = RHI::Null;
        }
    }
    ...
    void CD3D12API::DestroyCommandBufferPoolImpl( RHI::CommandBufferPool* pInOut, const void* pAllocator )
    {
        ...
        if( pNativeCommandBufferPool != NativeAPI::Null )
        {
            for( auto& NativeCommandListWithAllocator: pNativeCommandBufferPool->vNativeCommandListsWithAllocators )
            {
                ...
                NativeCommandListWithAllocator.pAllocator->Release();
                NativeCommandListWithAllocator.pAllocator = NativeAPI::Null;
            }

            Memory::DestroyObject( &HeapAllocator, &pNativeCommandBufferPool );
            *pInOut = RHI::Null;
        }
    }
                                                             ...
                                                             const void*                pAllocator )
    {
...

        {
            ...
            pPool->descriptorSize = m_pImplementation->m_hDevice->GetDescriptorHandleIncrementSize( nativePoolType );
    ...
    void CD3D12API::DestroyDescriptorPoolImpl( RHI::DescriptorPool* pInOut, const void* pAllocator )
    {
        ...
        if( pNativeDescriptorPool != NativeAPI::Null )
        {
            pNativeDescriptorPool->DescriptorSetMemMgr.Destroy();

            pNativeDescriptorPool->pHeap->Release();
            pNativeDescriptorPool->pHeap = NativeAPI::Null;

            Memory::DestroyObject( &HeapAllocator, &pNativeDescriptorPool );
            *pInOut = RHI::Null;
        }
    }
    ...
    void CD3D12API::DestroyDescriptorSetLayoutImpl( RHI::DescriptorSetLayout* pInOut, const void* pAllocator )
    {
        ...
        {
            Memory::DestroyObject( &HeapAllocator, &pNativeDescriptorSetLayout );
            *pInOut = RHI::Null;
        }
    }
                                                     ...
                                                     RHI::DescriptorSet*            pOutNativeDescriptorSets )
    {
        ...
        {
            pOutNativeDescriptorSets[ layoutIndex ] = RHI::Null;
            // ToNative() nie zwraca pointera na memory!!!
            // NativeAPI::DescriptorSet pCurrentSet    = ToNative( pOutNativeDescriptorSets[ layoutIndex ] );
            NativeAPI::DescriptorSet pCurrentSet = NativeAPI::Null;
            ...
            const NativeAPI::DescriptorSetLayout pLayout = ToNative( EngineDescriptorSetInfo.phLayouts[ layoutIndex ] );
            ...
            auto firstSlotIndex = pNativePool->SlotMgr.AllocateSlots( pLayout->numSlots );
            if( firstSlotIndex != UNDEFINED_U32 )
            {
                pCurrentSet->PoolSlots             = { firstSlotIndex, pLayout->numSlots };
                pCurrentSet->pPool                 = pNativePool;
    ...
    void CD3D12API::UpdateImpl( const RHI::DescriptorSet& hDDISet, const SUpdateBindingsHelper& Info )
    {
        ...
        const UINT                      srvDescriptorSize =
            m_pImplementation->m_hDevice->GetDescriptorHandleIncrementSize( D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV );
        ...
        for( auto& Binding: Info.vTexs )
        {
            ...
            for( uint32_t i = 0; i < Binding.count; ++i )
            {
                ...
                m_pImplementation->m_hDevice->CreateUnorderedAccessView( NativeTexture, nullptr, &UavDesc, hCpu );
            }
        }
            ...
            for( uint32_t index = 0; index < Binding.count; index++ )
            {
                ...
                switch( Binding.type )
                {
                    ...
                    case BINDING_TYPE::DYNAMIC_CONSTANT_BUFFER:
                        ...
                        m_pImplementation->m_hDevice->CreateConstantBufferView( &cbvDesc, hCpuDescriptorHandle );
                        break;
                    ...
                    case BINDING_TYPE::DYNAMIC_BUFFER:
                        ...
                        m_pImplementation->m_hDevice->CreateShaderResourceView(
                            pNativeResource, &srvDesc, hCpuDescriptorHandle );
                        break;
    ...
    RHI::Pipeline CD3D12API::CreatePipelineImpl( const SPipelineDesc& EngineDesc, const void* pAllocator )
    {
        ...
        else
        {
            ...
            pNativePipeline->VertexBufferStride = 0;
            for( uint32_t index = 0; index < EngineDesc.InputLayout.vVertexAttributes.GetCount(); index++ )
            {
                const auto& EngineVertexAttribute    = EngineDesc.InputLayout.vVertexAttributes[ index ];
                pNativePipeline->VertexBufferStride += EngineVertexAttribute.stride;
            }

            // Get a topology for vertex buffers bound to pipeline.
            pNativePipeline->PrimitiveTopology = Map::GetPrimitiveTopology( EngineDesc.InputLayout.topology );
    ...
    void CD3D12API::DestroyPipelineImpl( RHI::Pipeline* pInOut, const void* pAllocator )
    {
        ...
        {
            // TODO(blturkot): Handle different pipelines, eg.: raytracing
            pNativePipeline->Graphics->Release();
            pNativePipeline->Graphics = NativeAPI::Null;

            Memory::DestroyObject( &HeapAllocator, &pNativePipeline );
            *pInOut = RHI::Null;
        }
    }
    ...
    RHI::PipelineLayout CD3D12API::CreatePipelineLayoutImpl( const SPipelineLayoutDesc& Desc, const void* pAllocator )
    {
        ...
        {
            const RHI::DescriptorSetLayout& hDescriptorSetLayout = m_pCtx->GetDescriptorSetLayout( layout );
            vRootParameters.PushBack( ToNative( hDescriptorSetLayout )->RootParameter );
        }
        ...
        if( FAILED( m_pImplementation->m_hDevice->CreateRootSignature( Helper::GetNodeMask(),
        ...
        // they don't leak (they are not device children but still ref-counted COM objects).
        if( pSignatureBlob != nullptr )
        {
    ...
    void CD3D12API::DestroyPipelineLayoutImpl( RHI::PipelineLayout* pInOut, const void* pAllocator )
    {
        ...
        {
            pNativePipelineLayout->Release();
            pNativePipelineLayout = NativeAPI::Null;
    ...
    void CD3D12API::DestroySamplerImpl( RHI::Sampler* pInOut, const void* pAllocator )
    {
        ...
        if( pNativeSampler != NativeAPI::Null )
        {
            Memory::DestroyObject( &HeapAllocator, &pNativeSampler );
            *pInOut = RHI::Null;
        }
    }
    ...
    void CD3D12API::BindImpl( const SBindPipelineInfo& Info )
    {
        ...
        if( Info.pPipeline->GetType() == PIPELINE_TYPE::COMPUTE )
        {
            ...
            pNativeCmdBuffer->SetPipelineState( pNativePipelineState->Compute );
        }
        else
        {
            pNativeCmdBuffer->SetGraphicsRootSignature( pNativePipelineLayout );
            pNativeCmdBuffer->SetPipelineState( pNativePipelineState->Graphics );
            pNativeCmdBuffer->IASetPrimitiveTopology( pNativePipelineState->PrimitiveTopology );
        }
    }
    ...
    void CD3D12API::BindImpl( const SBindVertexBufferInfo& EngineInfo )
    {
        ...
        NativeVertexBufferView.StrideInBytes  = pNativePipeline->VertexBufferStride;
        NativeVertexBufferView.SizeInBytes    = (UINT)NativeResourceDesc.Width - EngineInfo.offset;
    ...
    void CD3D12API::FreeImpl( RHI::MemoryHeap* phMemory, const void* )
    {
        ...
        if( pNativeMemory == NativeAPI::Null )
        {
            return;
        }

        // RHI::MemoryHeap must wrap a genuine ID3D12Heap* which is a COM object and therefore
        // pointer-aligned. A sub-allocation handle (pool base + offset) is not aligned and must
        // never reach here - releasing it would corrupt memory / crash. Guard against passing an
    ...
    Result CD3D12API::AllocateImpl( const SAllocateMemoryDesc& Desc, SAllocateMemoryData* pOut )
    {
        ...
        NativeAPI::MemoryHeap pNativeMemory = nullptr;
        if( FAILED( hr = m_pImplementation->m_hDevice->CreateHeap( &heapDesc, IID_PPV_ARGS( &pNativeMemory ) ) ) )
        {
            if( hr == E_OUTOFMEMORY )
            {
    ...
    MEMORY_HEAP_TYPE CD3D12API::GetMemoryHeapTypeImpl( MEMORY_USAGE usage ) const
    {
        /*
        MEMORY_HEAP_TYPE:
        - CPU,          // CPU accessible only, no GPU access.
        - GPU,          // GPU only (DX12 DEFAULT heap).
        - UPLOAD,       // CPU write-combine, GPU read (DX12 UPLOAD heap).
        - CPU_CACHED,   // CPU cached read-back, GPU write (DX12 READBACK heap).
        - CPU_COHERENT, // CPU coherent (no flush) accessible, no dedicated GPU heap.
        - OTHER,        // Anything that doesn't fit above (DX12 CUSTOM heap).
        */

        // Decode the relevant usage bits. This mirrors the DX12 heap selection done in
        // Convert::GetMemoryHeapDesc so both stay consistent.
        const bool cpuAccess  = ( usage & MemoryUsages::CPU_ACCESS ) != 0;
        const bool gpuAccess  = ( usage & MemoryUsages::GPU_ACCESS ) != 0;
        const bool cpuNoFlush = ( usage & MemoryUsages::CPU_NO_FLUSH ) != 0; // write-combine / coherent
        const bool cpuCached  = ( usage & MemoryUsages::CPU_CACHED ) != 0;   // read-back

        MEMORY_HEAP_TYPE result;

        if( gpuAccess && !cpuAccess )
        {
            // GPU only -> DX12 DEFAULT heap.
            result = MemoryHeapTypes::GPU;
        }
        else if( gpuAccess && cpuAccess )
        {
            // Shared CPU/GPU access. Prefer the specialized DX12 heaps.
            if( cpuCached )
            {
                // CPU-cached read-back path -> DX12 READBACK heap.
                result = MemoryHeapTypes::CPU_CACHED;
            }
            else if( cpuNoFlush )
            {
                // CPU write-combine upload path -> DX12 UPLOAD heap.
                result = MemoryHeapTypes::UPLOAD;
            }
            else
            {
                // Shared access without a specific caching preference -> CUSTOM heap.
                result = MemoryHeapTypes::OTHER;
            }
        }
        else if( cpuAccess )
        {
            // CPU accessible only (no GPU access).
            // Coherent (no flush) CPU memory is reported separately from plain CPU memory.
            result = cpuNoFlush ? MemoryHeapTypes::CPU_COHERENT : MemoryHeapTypes::CPU;
        }
        else
        {
            // No CPU or GPU access bits specified -> fall back to CUSTOM heap.
            result = MemoryHeapTypes::OTHER;
        }
    ...
    void CD3D12API::BarrierImpl( const RHI::CommandBuffer& hCommandBuffer, const SBarrierInfo& Info )
    {
        ...
        {
            ToNative( hCommandBuffer )->ResourceBarrier( vBarriers.GetCount(), vBarriers.GetData() );
        }
        else
        {
    ...
    void CD3D12API::CopyImpl( const RHI::CommandBuffer& hDDICmdBuffer, const SCopyTextureInfoEx& EngineInfo )
    {
        ...
        Destination.pResource        = pDstTexture;
    ...
    Result CD3D12API::PresentImpl( const SPresentData& Info )
    {
        ...
        {
            NativeAPI::Fence      fence      = ToNative( Info.vWaitForFences[ index ] );
            NativeAPI::FenceValue fenceValue = Info.vWaitForFenceValues[ index ];
            fence->Wait( pNativeQueue, fenceValue );
            // pNativeQueue->Wait( fence->pObject, fenceValue );
        }

        // Present all swap chains
        for( uint32_t index = 0; index < Info.vSwapchains.GetCount(); index++ )
        {
            auto pSwapChain = ToNative( Info.vSwapchains[ index ] );
            VKE_ASSERT( pSwapChain != NativeAPI::Null );

            // TODO(blturkot): Support VSync but we don't have full information here.
            UINT syncInterval = 0;
            UINT presentFlags = 0;

            if( syncInterval == 0 && SImplementation::SDeviceFeatures::sTearingSupported )
            {
    ...
    Result CD3D12API::CreateSwapChainImpl( const SSwapChainDesc& Desc, const void*, SDDISwapChain* pOut )
    {
        ...
        NativeAPI::Queue pQueue = ToNative( m_DeviceProperties.vQueueFamilies[ Desc.queueFamilyIndex ].vQueues[ 0 ] );
        ...
        NativeAPI::SwapChain pSwapChain = reinterpret_cast< NativeAPI::SwapChain >( pSwapChain1 );
        // Update out handle to higher SwapChain ptr. Similar to .As() on ComPtr
        pOut->hSwapChain = FromNative< RHI::SwapChain >( pSwapChain );

        // DXGI has already bound the swapchain to the window.
        IDXGIOutput* pPresentOutput;
        pSwapChain->GetContainingOutput( &pPresentOutput );
        NativeAPI::PresentSurface pPresentSurface = reinterpret_cast< NativeAPI::PresentSurface >( pPresentOutput );
        pOut->hSurface                            = FromNative< RHI::PresentSurface >( pPresentSurface );
        ...
        {
            pSwapChain->SetColorSpace1( dxgiColorSpace );
        }

        for( UINT i = 0; i < backBufferCount; i++ )
        {
            NativeAPI::Texture NativeBackBufferTexture = NativeAPI::Null;
            ...
            pOut->vImages.PushBack( FromNative< RHI::Texture >( NativeBackBufferTexture ) );
            pOut->vImageViews.PushBack( RHI::Null );
        }

        // Update size from SwapChain after creation
        UINT width = 0, height = 0;
        pSwapChain->GetSourceSize( &width, &height );
    ...
    void CD3D12API::DestroySwapChainImpl( SDDISwapChain* pInOut, const void* )
    {
        ...
        {
            auto pNativeTexture = ToNative( pInOut->vImages[ index ] );
            if( pNativeTexture != NativeAPI::Null )
            {
                pNativeTexture->Release();
            }
        ...
        pInOut->vImages.Clear();
        pInOut->vImageViews.Clear();

        auto pNativeSurface = ToNative( pInOut->hSurface );
        if( pNativeSurface != NativeAPI::Null )
        {
            pNativeSurface->Release();
            pInOut->hSurface = RHI::Null;
        }

        auto pNativeSwapChain = ToNative( pInOut->hSwapChain );
        if( pNativeSwapChain != NativeAPI::Null )
        {
    ...
    void CD3D12API::SetObjectDebugNameImpl( const uint64_t& handle, const uint32_t& objType, cstr_t pName ) const
    {
        ...
        switch( objType )
        {
            ...
            /*case ApiObjectTypes::BUFFER_VIEW:
                ( (RHI::BufferView)handle )->SetName( buffer );
                break;
            ...
            case ApiObjectTypes::PIPELINE:
                ( (NativeAPI::Pipeline)handle )->Graphics->SetName( buffer );
                break;
                /* case ApiObjectTypes::DESCRIPTOR_SET:
                     ( (RHI::DescriptorSet)handle )->SetName( buffer );
                     break;*/
    ...
    Result CD3D12API::WaitForQueueImpl( const RHI::Queue& hQueue )
    {
        ...
        out = pNativeFence->Wait( drainValue );
