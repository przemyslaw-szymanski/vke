    ...
    namespace RenderSystem
    {
        namespace D3D12Trait
        {
            ...
            using QueueFamilyPropertyArray
                = Utils::TCDynamicArray<VkQueueFamilyProperties, DEFAULT_QUEUE_FAMILY_PROPERTY_COUNT>;
            ...
            class VKE_API CDDI
            {
              ...
              public:
                static const GlobalICD& GetGlobalICD()
                {
                    return sGlobalICD;
                }
                static const InstanceICD& GetInstantceICD()
                {
                    return sInstanceICD;
                }
                ...
                void DestroyDevice();
                const DeviceICD& GetDeviceICD() const
                {
                    return m_ICD;
                }
                const DeviceICD& GetICD() const
                {
                    return m_ICD;
                }
                static Result LoadICD( const SDDILoadInfo& Info, SDriverInfo* pOut );
                static void CloseICD();
                static const SDDIExtension& GetInstanceExtensionInfo( cstr_t pName );
                const DDIDevice& GetDevice() const
                {
                    return m_hDevice;
                }
                ...
                const DDIAdapter& GetAdapter() const
                {
                    return m_hAdapter;
                }
                ...
                Result GetTextureMemoryRequirements( const DDITexture& hTexture,
                ...
                template<RESOURCE_TYPE Type> Result Bind( const SBindMemoryInfo& Info );
                ...
                // void            BeginRenderPass( const DDICommandBuffer& hCommandBuffer, const SBeginRenderPassInfo&
                // Info ); void            EndRenderPass( const DDICommandBuffer& hCommandBuffer );
                ...
                void SetEvent( const DDICommandBuffer& hDDICmdBuffer, const DDIEvent& hDDIEvent,
                               const PIPELINE_STAGES& stages );
                void Reset( const DDIEvent& hDDIInOut );
                void Reset( const DDICommandBuffer& hDDICmdBuffer, const DDIEvent& hDDIEvent,
                            const PIPELINE_STAGES& stages );
            ...
            template<RESOURCE_TYPE Type> Result CDDI::Bind( const SBindMemoryInfo& Info )
            {
