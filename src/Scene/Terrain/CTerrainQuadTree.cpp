...
#define VKE_LOAD_VEC4_XZY( _vec4 )                                                                                     \
    ...
    {                                                                                                                  \
                                      ...
                                      const Math::CVector4& vec4ViewPos, Math::CVector4* pOut )
        {
            ...
            const Math::CVector4 aViewPos[ 3 ] = { Math::CVector4{ vec4ViewPos.x },
        ...
        void CTerrainQuadTree::_UpdateWorldSize()
        {
            // Calc world corners
            {
                ...
                CalcNodePosition(
                    m_Desc.vecCenter, m_pTerrain->m_vecExtents.x, &m_avecWorldCorners[ ChildNodeIndices::LEFT_TOP ] );
                m_avecWorldCorners[ ChildNodeIndices::LEFT_BOTTOM ] =
                    m_avecWorldCorners[ ChildNodeIndices::LEFT_TOP ] - Math::CVector3( 0, 0, (float)WorldSize.height );
                ...
                m_avecWorldCorners[ ChildNodeIndices::RIGHT_BOTTOM ] =
                    m_avecWorldCorners[ ChildNodeIndices::RIGHT_TOP ] - Math::CVector3( 0, 0, (float)WorldSize.height );
            }
        }
        ...
        void CTerrainQuadTree::_InitChildNodesSIMD( const SInitChildNodeLevel& Info )
        {
            ...
            // SIMD
            {
                ...
                // depth
                Math::CVector4::Load(
                    Info.vecParentSizes.floats, &vec4ParentCenterX, &vec4ParentCenterZ, &vec4ParentExtentsWD );
        ...
        void MapRootNodeSpaceToGridSpace( const CTerrainQuadTree::SNodeLevel& NodeLevel, const SRootInfoSIMD& RootInfo,
                                          Math::CVector4* pOut )
        {
            ...
            Math::CVector4 vec4Tmp1 =
                Math::CVector4::Abs( ( RootInfo.vec4RootCornerXs - vec4Xs ) / RootInfo.vec4TileSize );
            Math::CVector4 vec4Tmp2 =
                Math::CVector4::Abs( ( RootInfo.vec4RootCornerZs - vec4Zs ) / RootInfo.vec4TileSize );
        ...
        void CTerrainQuadTree::_CalcLODsSIMD( const SNodeLevel& ChildNodes, const SNode& Root, const SViewData& View,
                                              float lodTreshold )
        {
            ...
            }
            {
                ...
                CalcNearestSpherePoints(
                    ChildNodes.aAABBCenters, boundingSphereRadius, vec4ViewPosition, aNearestPoints );
                CalcErrors( aNearestPoints,
                            m_Desc.vertexDistance,
                            ChildNodes.level,
                            m_Desc.lodCount,
                            View,
                            &vecErrors,
                                                    ...
                                                    const uint8_t lodCount )
        {
            ...
            if( currLevel < lodCount )
            {
                ...
                for( uint8_t i = 0; i < 4; ++i )
                {
                    ...
                    Node.ahChildren[ 3 ].handle = UNDEFINED_U32;
                    Math::CVector4::Mad(
                        aVectors[ i ], NodeData.vec4Extents, NodeData.vec4ParentCenter, &vecChildCenter );
...
namespace VKE::Scene::Terrain
{
    ...
    vke_force_inline std::array< Math::CVector4, 2 > CalcChildrenPositions2( Math::CVector4 Position, float size )
    {
        ...
        const std::array< Math::CVector4, 2 > aRet = { Math::CVector4::LoadXYToZW( Position.x, Position.x + half ),
                                                       Math::CVector4::LoadXYToZW( Position.y, Position.y - half ) };
        return aRet;
    }

    vke_force_inline ExtentI32 CalcNodeCenterPoint( const ExtentI32& NodePos, uint32_t nodeHalfSize )
    {
        return { NodePos.x + (int32_t)nodeHalfSize, NodePos.y - (int32_t)nodeHalfSize };
    }

    vke_force_inline ExtentI32 CalcChildNodePosition( CHILD_INDEX childNodeIndex, const ExtentI32& ParentNodePosition,
                                                      int32_t childSize )
    {
        ...
        return { ParentNodePosition.x + childSize * aOffsets[ childNodeIndex ].x,
    ...
    Result CQuadTree::_Resize( const ExtentI32& NewPos, uint32_t newSize )
    {
        ...
        if( m_maxDepth < newMaxDepth )
        {
            ...
            if( pRoot->aChildren[ 0 ] )
            {
                ...
                {
                    ExtentI32 ChildPos =
                        CalcChildNodePosition( (CHILD_INDEX)i, m_RootData.TopLeftCornerPosition, childSize );
                    NodeHandle hParent      = _CreateNode( NewPos, newSize, m_RootData.hNode, ChildPos, depthDiff );
    ...
    CQuadTree::NodeHandle CQuadTree::_CreateNode( ExtentI32 ParentPos, uint32_t parentSize, NodeHandle hParent,
    ...
    {
        NodeHandle hParent = _CreateNode(
            m_RootData.TopLeftCornerPosition, m_RootData.size, m_RootData.hNode, ChildDesc.Position, m_maxDepth );
