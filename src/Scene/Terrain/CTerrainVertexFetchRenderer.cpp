...
namespace VKE
{
    namespace Scene
    {
        ...
        static cstr_t g_pGLSLTerrainVS = R"(#version 450 core
...
#define TILE_VERTEX_COUNT 32.0
            ...
            layout( location = 0 ) out vec4 oColor; layout( location = 1 ) out vec2 oTexcoord;
        ...
        static cstr_t g_pGLSLTerrainPS = R"(#version 450 core

            // layout(set = 1, binding = 1) uniform sampler2D HeightmapTexture;
            layout( set = 1, binding = 1 ) uniform sampler   VertexFetchSampler;
            layout( set = 1, binding = 2 ) uniform texture2D HeightmapTextures[ 10 ];

            layout( location = 0 ) in vec4 iColor; layout( location = 1 ) in vec2 iTexcoord;
...
#include "Shaders/VertexFetch.ps.hlsl.h"
                                                                 ...
                                                                 RenderSystem::CommandBufferPtr pCommandBuffer )
        {
            ...
            m_ahVertexBuffers[ DrawTypes::QUADS ] =
                m_ahVertexBuffers[ DrawTypes::TRIANGLES ]; // The same vertex buffer for both types
            return ret;
        }
...
#define CALC_IDX_11( _x, _y, _w ) CALC_XY( _x + 1, _y, _w )
        ...
        Result CTerrainVertexFetchRenderer::_CreateBindings( RenderSystem::CommandBufferPtr pCommandBuffer )
        {
            ...
            maxTextures          = Math::Min( maxTextures, CTerrain::MAX_TEXTURE_COUNT );
            {
                ...
                g_TileBindingDesc.AddTextures(
                    2, RenderSystem::PipelineStages::ALL, (uint16_t)m_pTerrain->m_vHeightmapTexViews.GetCount() );
                // Heightmap normal textures
                g_TileBindingDesc.AddTextures(
                    3, RenderSystem::PipelineStages::ALL, (uint16_t)m_pTerrain->m_vHeightmapNormalTexViews.GetCount() );
                g_TileBindingDesc.AddTextures(
                    4, RenderSystem::PipelineStages::ALL, (uint16_t)m_pTerrain->m_vSplatmapTexViews.GetCount() );
                g_TileBindingDesc.AddTextures(
                    6,
                    RenderSystem::PipelineStages::ALL,
            ...
            // Instancing
            {
                ...
                // Heightmap textures
                g_InstanceBindingDesc.AddTextures(
                    3, RenderSystem::PipelineStages::ALL, (uint16_t)m_pTerrain->m_vHeightmapTexViews.GetCount() );
                // Heightmap normal textures
                g_InstanceBindingDesc.AddTextures(
                    4, RenderSystem::PipelineStages::ALL, (uint16_t)m_pTerrain->m_vHeightmapNormalTexViews.GetCount() );
                // Heightmap normal textures
                g_InstanceBindingDesc.AddTextures(
                    5, RenderSystem::PipelineStages::ALL, (uint16_t)m_pTerrain->m_vSplatmapTexViews.GetCount() );
                g_InstanceBindingDesc.AddTextures(
                    6,
                    RenderSystem::PipelineStages::ALL,
                    (uint16_t)m_pTerrain->m_avTextureViews[ CTerrain::TextureTypes::DIFFUSE ].GetCount() );
        ...
        uint32_t CTerrainVertexFetchRenderer::_CreateTileBindings( uint32_t resourceIndex )
        {
        ...
        Result CTerrainVertexFetchRenderer::_CreateConstantBuffers( RenderSystem::CDeviceContext* pCtx )
        {
            ...
            uint32_t maxTileCount        = maxTileCountInRoot * m_pTerrain->m_TerrainInfo.RootCount.width *
                                           m_pTerrain->m_TerrainInfo.RootCount.height;
            (void)maxTileCount;
                                                                                ...
                                                                                RenderSystem::CDeviceContext* pCtx ) //
        {
            ...
            const ShaderCompilerString TessellationFactorDistancePowMultiplier = ShaderCompilerString( 1 );
            ...
            auto pFile                 = m_pTerrain->m_pScene->GetDeviceContext()
                                             ->GetRenderSystem()
                                             ->GetEngine()
                                             ->GetManagers()
                                             .pFileMgr->LoadFile( FileDesc );
...

            {
                ...
                Pipeline.Rasterization.Polygon.frontFace = VKE_SCENE_TERRAIN_CCW
                                                               ? RenderSystem::FrontFaces::COUNTER_CLOCKWISE
                                                               : RenderSystem::FrontFaces::CLOCKWISE;
            }
        ...
        void CTerrainVertexFetchRenderer::_SortDrawcalls()
        {
            ...
            std::sort( vDrawcalls.GetData(),
                       vDrawcalls.GetData() + vDrawcalls.GetCount(),
            ...
            }
            {
                ...
                if( !vLODData.IsEmpty() )
                {
                    ...
                    for( uint32_t i = 0; i < vLODData.GetCount(); ++i )
                    {
                        ...
                        PerDrawData.vertexDiff                  = Pack4BytesToUint( LODData.DrawData.topVertexDiff,
                                                                                    LODData.DrawData.bottomVertexDiff,
                                                                                    LODData.DrawData.leftVertexDiff,
                                                                                    LODData.DrawData.rightVertexDiff );
        ...
        void vke_force_inline CalcTilePositions( float tileSize, const Math::CVector3& vecLeftTopTerrainCorner,
                                                 const ExtentF32& TileIndex, Math::CVector3* pvecCenterOut,
                                                 Math::CVector3* pvecTopLeftCorner )
        {
        ...
        /*void CTerrainVertexFetchRenderer::_UpdateTileConstantBufferData( const SPerDrawConstantBufferData& Data,
            uint32_t drawIdx )
        {
            auto pData = (SPerDrawConstantBufferData*)&m_vConstantBufferData[m_pConstantBuffer->CalcOffset( 1,
        (uint16_t)drawIdx )]; *pData = Data;
        }*/
        ...
        void CTerrainVertexFetchRenderer::_RenderInstancing( RenderSystem::CommandBufferPtr pCommandBuffer,
                                                             CScene*                        pScene )
        {
                ...
                if( g_draws )
                    ...
                    for( uint32_t i = 0; i < m_vInstnacingInfos.GetCount(); ++i )
                    {
                        ...
                        if( Info.pPipeline->IsResourceReady() )
                        {
                            ...
                            pCommandBuffer->Bind(
                                1, m_ahPerInstancedDrawDescSets[ m_lastBindingsUpdateIndex ], offsets, 2u );
                                ...
                                if( pLastPipeline != pPipeline )
                                {
                                    ...
                                    if( g_bindBuffers )
                                        ...
                                        if( !isPerFrameBound )
                                        {
                                            ...
                                            pCommandBuffer->Bind( m_ahIndexBuffers[ drawType ], 0u );
                                            pCommandBuffer->Bind( m_ahVertexBuffers[ drawType ],
                                                                  m_vDrawLODs[ 0 ].vertexBufferOffset );
                                            pCommandBuffer->Bind( 0,
                                                                  m_ahPerFrameDescSets[ m_backBufferIndex ],
