...
namespace VKE::Scene
{
        ...
        RenderSystem::CommandBufferPtr pCmdBuffer )
    {
        ...
        m_SubTileDesc.meshletSize = m_SubTileDesc.meshletDistance / ((float)( m_MeshletDesc.vertexCountInRow - 1 )/m_MeshletDesc.vertexCountInRow);
        ...
        //m_MeshletDesc.vertexDistance = static_cast<float>( TerrainDesc.TileSize.min ) / ( m_TileDesc.meshletCountInRow );
        ...
        UpdateInfo.AddBinding( 3u, 0u, m_pTileBuffer->GetSize(),
            m_pTileBuffer->GetHandle(),
                               RenderSystem::BindingTypes::BUFFER );
        UpdateInfo.AddBinding( 4u, 0u, m_pDebugBuffer->GetSize(),
            m_pDebugBuffer->GetHandle(),
            RenderSystem::BindingTypes::BUFFER );
    ...
    Result CTerrainMeshShadingRenderer::_CreateMeshletBuffer(RenderSystem::CommandBufferPtr pCmdBuffer)
    {
        ...
        const float meshletDistance = m_SubTileDesc.meshletDistance; //m_TileDesc.meshletSize - TerrainDesc.vertexDistance*1;
    ...
    Result CTerrainMeshShadingRenderer::_CreatePipeline(RenderSystem::CommandBufferPtr pCmdBuff)
    {
        ...
        if( CurrState.RenderPass.hash != m_renderPassHash )
        {
            ...
            const RenderSystem::SShaderDesc::DefineArray vDefines =
            {
                ...
                { ( L"TOTAL_VERTEX_COUNT" ), ShaderCompilerString( m_MeshletDesc.vertexCountInRow * m_MeshletDesc.vertexCountInRow ).GetData() },
     ...
     Result CTerrainMeshShadingRenderer::_CreateVertexBuffer( RenderSystem::CommandBufferPtr pCmdBuffer )
    {
        ...
        if( hBuffer != INVALID_HANDLE )
        {
            ...
            RenderSystem::SDebugInfo DebugInfo
                = { .pText = "Upload MeshShader VertexBuffer", .Color = RenderSystem::SColor::GREEN };
...
#if VKE_RENDER_SYSTEM_DEBUG
            RenderSystem::SDebugInfo DebugInfo
                = { .pText = "Upload MeshShader TriangleBuffer", .Color = RenderSystem::SColor::GREEN };
            UpdateInfo.pDebugInfo = &DebugInfo;
#endif
            ret = pCmdBuffer->GetContext()->UpdateBuffer( pCmdBuffer, UpdateInfo, &hBuffer );
            if( VKE_SUCCEEDED( ret ) )
            {
                BuffDesc.Buffer.size = sizeof( float ) * 4 * m_SubTileDesc.totalMeshletCount * m_TaskDesc.totalSubTileCount;
