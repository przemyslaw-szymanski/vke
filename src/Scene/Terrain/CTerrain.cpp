...
namespace VKE
{
    namespace Scene
    {
        ...
        CalcNodePosition( const Math::CVector4& vec4Center,
                          const float& nodeExtents, Math::CVector3* pOut )
        {
        ...
        void vke_force_inline
        CalcNodePosition( const Math::CVector3& vec3Center,
                          const float& nodeExtents, Math::CVector3* pOut )
        {
        ...
        bool CTerrain::CheckDesc( const STerrainDesc& Desc ) const
        {
            ...
            if( Desc.vertexDistance < 1.0f )
            {
                ...
                // power of 2
                if( Desc.vertexDistance != 0.125f ||
                    Desc.vertexDistance != 0.25f ||
                    Desc.vertexDistance != 0.5f )
                {
                    ret = false;
                }
            }
        ...
        Result CTerrain::_Create( const STerrainDesc& Desc, RenderSystem::CommandBufferPtr pCommandBuffer )
        {
            ...
            if( strcmp( m_Desc.Renderer.pName,
                        TERRAIN_VERTEX_FETCH_RENDERER_NAME ) == 0 )
            {
                m_pRenderer = VKE_NEW CTerrainVertexFetchRenderer( this );
            }
            else if( strcmp( m_Desc.Renderer.pName,
                             TERRAIN_MESH_SHADING_RENDERER_NAME ) == 0 )
            {
            }
            if( m_pRenderer == nullptr )
            {
            ...
            m_maxVisibleTiles = Math::Min(
                m_maxTileCount, ( uint16_t )CalcMaxVisibleTiles( m_Desc ) );
...
ERR:
        ...
        CTerrain::_CreateDummyResources( RenderSystem::CommandBufferPtr pCommandBuffer )
        {
            Result ret = VKE_FAIL;
            {
                ...
                if( hTex != INVALID_HANDLE )
                {
                    ...
                    {
                        VKE_LOG_ERR(
                            "Unable to create terrain dummy texture view." );
                    }
                }
                else
                {
        ...
        Core::ImageSize CalcRequiredHeightmapSize( const ExtentU16& RootCount,
                                                   uint16_t rootSize )
        {
            Core::ImageSize Ret = { RootCount.width * rootSize + 1u,
                                    RootCount.height * rootSize + 1u };
        ...
        Result CreateTextures( RenderSystem::CDeviceContext* pCtx,
                               const ExtentU16& RootCount, uint16_t rootSize,
                               Core::CImageManager* pImgMgr,
                               const Core::ImageHandle& hImg,
                               const STerrainDesc& Desc,
                               CTerrain::TextureArray* pInOut )
        {
            ...
            const Core::ImageSize RequiredSize =
                CalcRequiredHeightmapSize( RootCount, rootSize );
            if( VKE_SUCCEEDED(
                    pImgMgr->Resize( RequiredSize, &SliceInfo.hSrcImage ) ) )
            {
                for( uint16_t y = 0; y < RootCount.y; ++y )
                {
                    Region.Size.height =
                        rootSize + 1; // ((y + 1) < m_TerrainInfo.RootCount.y);
                    ...
                    for( uint16_t x = 0; x < RootCount.x; ++x )
                    {
                        ...
                        Region.Size.width =
                            rootSize +
                            1; // ((x + 1) < m_TerrainInfo.RootCount.x);
                        Region.Offset.x = x * rootSize;
                        SliceInfo.vRegions.PushBack( Region );
                    }
                }
            }
            Utils::TCDynamicArray<Core::ImageHandle> vImages(
                SliceInfo.vRegions.GetCount() );
                ...
                if( pInOut->Reserve( vImages.GetCount() ) )
                {
                    for( uint32_t i = 0; i < vImages.GetCount(); ++i )
                    {
                        ...
                        Math::Map1DarrayIndexTo2DArrayIndex(
                            i, RootCount.x, RootCount.y, &x, &y );
                        vke_sprintf( name, 128,
                                     "vke_slice_heightmap%d_%d-%d.png",
                                     rootSize, x, y );
                        ...
                        VKE_RENDER_SYSTEM_SET_DEBUG_NAME( TexDesc.Texture,
                                                          name );
                        RenderSystem::TextureHandle hTex =
                            pCtx->CreateTexture( TexDesc );
                        if( hTex != INVALID_HANDLE )
                        {
            ...
            {
                VKE_LOG_ERR(
                    "One ore more texture was not created from split image." );
            }
            return ret;
        }
        ...
        Result CTerrain::_LoadTextures( RenderSystem::CDeviceContext* pCtx, const STerrainUpdateBindingData& Data )
        {
            ...
            {
                m_vHeightmapTextures[ Data.index ] = ( hTex );
            }
            }
            {
        ...
        void
        CTerrain::_GetBindingDataForRootNode( const uint32_t& rootNodeIdx,
    ...
    namespace Scene
    {
        ExtentU32 vke_force_inline CalcXY( const uint32_t idx,
        ...
        uint32_t vke_force_inline CalcTileCountForLod( uint8_t startLOD,
                                                       uint8_t LODCount )
        {
            uint32_t ret = 0;
        ...
        ExtentU8 CTerrainQuadTree::CalcLODCount( const STerrainDesc& Desc,
                                                 uint16_t maxHeightmapSize,
                                                 uint8_t maxLODCount )
        {
            ExtentU8 Ret;
            ...
            uint8_t maxLOD = 0;
            for( uint32_t size = rootSize; size >= Desc.TileSize.min;
                 size >>= 1, maxLOD++ )
            {
            }
        ...
        void CTerrainQuadTree::_CalcTerrainInfo( const SCalcTerrainInfo& Info,
                                                 STerrainInfo* pOut )
        {
            const auto& Desc = *Info.pDesc;
            const auto LODCount = CalcLODCount(
                *Info.pDesc, ( uint16_t )Info.maxRootSize, Info.maxLODCount );
            pOut->maxLODCount = LODCount.max;
            // pOut->rootLOD = LODCount.min;
            // Calculate total root count
            // Terrain is a quad
            {
                pOut->RootCount.x = pOut->RootCount.y =
                    CalcRootCount( Desc, ( uint16_t )Info.maxRootSize );
            }
            const uint32_t totalRootCount =
                pOut->RootCount.width * pOut->RootCount.height;
            // Calc root size
            const uint32_t rootSize =
                ( uint32_t )Math::CalcPow2( pOut->maxLODCount - 1 ) *
                Desc.TileSize.min;
            // Calc max visible roots
            {
                uint32_t viewDistance =
                    Math::Max( rootSize, ( uint32_t )Desc.maxViewDistance );
                ...
                const uint32_t visibleRootCount =
                    ( uint32_t )( viewDistance / rootSize ) *
                    2; // get all roots along whole quad
                pOut->maxVisibleRootCount = Math::Min(
                    totalRootCount,
            ...
            // Calc total tile count for one root
            {
                const uint32_t nodeCount =
                    CalcTileCountForRoot( pOut->maxLODCount );
            ...
            // Calc total level count for child nodes for root
            {
                ...
                // l 0 (root) == 1
                pOut->childLevelCountForRoot =
                    CalcTileCountForRoot( pOut->maxLODCount - 1 );
            }
            ...
            // for all LODs
            {
                ...
                pOut->maxNodeCount = totalRootCount + nodeCountFor2x2 -
                                     4; // totalRootCount contains all roots
            }
        }
        void CTerrainQuadTree::_UpdateWorldSize()
        {
            // Calc world corners
            {
                const ExtentU32 WorldSize = {
                    ( uint32_t )( m_RootNodeCount.width * m_Desc.TileSize.max ),
                    ( uint32_t )( m_RootNodeCount.height * m_Desc.TileSize.max )
                };
                CalcNodePosition(
                    m_Desc.vecCenter, m_pTerrain->m_vecExtents.x,
                    &m_avecWorldCorners[ ChildNodeIndices::LEFT_TOP ] );
                m_avecWorldCorners[ ChildNodeIndices::LEFT_BOTTOM ] =
                    m_avecWorldCorners[ ChildNodeIndices::LEFT_TOP ] -
        ...
        Result CTerrainQuadTree::_Create( const STerrainDesc& Desc )
        {
            ...
            m_tileInRowCount =
                ( uint16_t )( m_terrainHalfSize / m_tileSize ) * 2;
            m_totalRootCount =
                ( uint16_t )( Info.RootCount.width * Info.RootCount.height );
            ...
            // const auto rootRowCount = CalcRootCount(Desc,
            // m_pTerrain->m_maxHeightmapSize); m_RootNodeCount = {rootRowCount,
            // rootRowCount};
            m_RootNodeCount = Info.RootCount;
            const auto vecMinWorldSize =
                m_Desc.vecCenter - m_pTerrain->m_vecExtents;
            const auto vecMaxWorldSize =
                m_Desc.vecCenter + m_pTerrain->m_vecExtents;
            _UpdateWorldSize();
            Math::CVector3& vecWorldTopLeftCorner =
                m_avecWorldCorners[ ChildNodeIndices::LEFT_TOP ];
            if( !m_vTextureIndices.Resize( m_RootNodeCount.x *
                                           m_RootNodeCount.y ) )
            {
                res = VKE_FAIL;
            }
            ...
            const bool nodeDataReady =
                m_vNodes.Resize( nodeCount ) &&
                m_vLODData.Reserve( nodeCount ) &&
                m_vNodeVisibility.Resize( nodeCount, true ) &&
                m_vBoundingSpheres.Resize( nodeCount ) &&
                m_vAABBs.Resize( nodeCount ) &&
                ...
                m_vChildNodeLevels.Resize( Info.childLevelCountForRoot *
                                           MAIN_ROOT_COUNT );
            ...
            if( nodeDataReady )
            {
                const auto vecRootNodeExtents =
                    m_pTerrain->m_vecExtents /
                    Math::CVector3( m_RootNodeCount.x, 1.0f,
                                    m_RootNodeCount.y );
                const auto vecRootNodeSize = vecRootNodeExtents * 2.0f;
                const auto RootAABB =
                    Math::CAABB( m_Desc.vecCenter, vecRootNodeExtents );
                ...
                for( uint16_t z = 0; z < m_RootNodeCount.y; ++z )
                {
                    for( uint16_t x = 0; x < m_RootNodeCount.x; ++x )
                    {
                        ...
                        Node.vec3Position.x =
                            vecWorldTopLeftCorner.x + vecRootNodeSize.x * x;
                        Node.vec3Position.y = 0;
                        Node.vec3Position.z =
                            vecWorldTopLeftCorner.z - vecRootNodeSize.z * z;
                        vecRootNodeCenter.x =
                            Node.vec3Position.x + vecRootNodeExtents.x;
                        vecRootNodeCenter.y = 0;
                        vecRootNodeCenter.z =
                            Node.vec3Position.z - vecRootNodeExtents.z;
                        Node.AABB = Math::CAABB( vecRootNodeCenter,
                                                 vecRootNodeExtents );
                        m_vAABBs[ Handle.index ] = Node.AABB;
                        m_vBoundingSpheres[ Handle.index ] =
                            Math::CBoundingSphere( Node.AABB.Center,
                                                   Node.boundingSphereRadius );
                    }
                }
                const auto& TmpRoot = m_vNodes[ 0 ];
                m_FirstLevelNodeBaseInfo.boundingSphereRadius =
                    TmpRoot.boundingSphereRadius * 0.5f;
                m_FirstLevelNodeBaseInfo.maxLODCount = m_maxLODCount;
                m_FirstLevelNodeBaseInfo.vecExtents =
                    TmpRoot.AABB.Extents * 0.5f;
                m_FirstLevelNodeBaseInfo.vec4Extents =
                    m_FirstLevelNodeBaseInfo.vecExtents;
                _ResetChildNodes();
                const uint32_t rootCount =
                    m_RootNodeCount.x * m_RootNodeCount.y;
                for( uint32_t i = 0; i < rootCount; ++i )
                {
                    auto& Root = m_vNodes[ i ];
                    SCreateNodeData NodeData;
                    NodeData.boundingSphereRadius =
                        Root.boundingSphereRadius * 0.5f;
                    NodeData.level = 1;
        ...
        void CTerrainQuadTree::_InitMainRoots( const SViewData& View )
        {
            ...
            const auto& vecViewPosition = View.vecPosition;
            {
                ...
                std::sort(
                    &m_vVisibleRootNodes[ 0 ],
                    &m_vVisibleRootNodes[ 0 ] + m_vVisibleRootNodes.GetCount(),
                    [ & ]( const SNode& Left, const SNode& Right ) {
                        const float leftDistance = Math::CVector3::Distance(
                            Left.AABB.Center, vecViewPosition );
                        const float rightDistance = Math::CVector3::Distance(
                            Right.AABB.Center, vecViewPosition );
            ...
            }
            {
                ...
                const uint32_t count =
                    Math::Min( m_vVisibleRootNodes.GetCount(), 4 );
                for( uint32_t i = 0; i < count; ++i )
                {
        ...
        void CTerrainQuadTree::_InitMainRootsSIMD( const SViewData& View )
        {
            ...
            }
            {
                ...
                // LevelInit.parentBoundingSphereRadius =
                // m_FirstLevelNodeBaseInfo.boundingSphereRadius;
                ...
                for( uint32_t i = 0; i < count; ++i )
                {
                    ...
                    LevelInit.vecParentSizes.z =
                        Root.AABB.Extents.x; // width == depth
                    LevelInit.parentLevel = Root.Handle.level;
                    LevelInit.childLevelIndex = _AcquireChildNodeLevel();
                    LevelInit.bindingIndex =
                        Root.DrawData.bindingIndex; // populate root binding for
                                                    // all child nodes
        ...
        void
        CTerrainQuadTree::_InitChildNodesSIMD( const SInitChildNodeLevel& Info )
        {
            ...
            static const Math::CVector4 vec4DirectionXs( -1.0f, 1.0f, -1.0f,
                                                         1.0f );
            static const Math::CVector4 vec4DirectionZs( 1.0f, 1.0f, -1.0f,
                                                         -1.0f );
            static const Math::CVector4 vec4Half( 0.5f );
            Math::CVector4& vec4CenterXs =
                ChildLevel.aAABBCenters[ SNodeLevel::X ];
            Math::CVector4& vec4CenterZs =
                ChildLevel.aAABBCenters[ SNodeLevel::Z ];
            Math::CVector4& vec4ChildExtentsWD =
            ...
            // SIMD
            {
                ...
                Math::CVector4::Mad( vec4DirectionXs, vec4ChildExtentsWD,
                                     vec4ParentCenterX, &vec4CenterXs );
                Math::CVector4::Mad( vec4DirectionZs, vec4ChildExtentsWD,
                                     vec4ParentCenterZ, &vec4CenterZs );
            }
            ChildLevel.boundingSphereRadius =
                Info.parentBoundingSphereRadius * 0.5f;
            ...
            if( childLevel < Info.maxLODCount )
            {
                ...
                ChildLevelInfo.parentBoundingSphereRadius =
                    ChildLevel.boundingSphereRadius;
                ChildLevelInfo.parentLevel = currLevel;
                ChildLevelInfo.vecParentSizes.z =
                    ChildLevel.aAABBExtents[ SNodeLevel::X ]
                        .x; // the same for all nodes
                ChildLevelInfo.bindingIndex = Info.bindingIndex;
                for( uint32_t i = 0; i < 4; ++i )
                {
                    ChildLevelInfo.vecParentSizes.x =
                        ChildLevel.aAABBCenters[ SNodeLevel::X ].floats[ i ];
                    ChildLevelInfo.vecParentSizes.y =
                        ChildLevel.aAABBCenters[ SNodeLevel::Z ].floats[ i ];
                    ChildLevelInfo.childLevelIndex = _AcquireChildNodeLevel();
                    ChildLevel.aChildLevelIndices[ i ] =
                        ChildLevelInfo.childLevelIndex;
            ...
            {
                ChildLevel.aChildLevelIndices[ 0 ] =
                    UNDEFINED_U32; // mark there are no more children
            }
        }
        void
        CTerrainQuadTree::_InitChildNodesSIMD( const SNodeLevel& ParentLevel )
        {
        }
        void
        CTerrainQuadTree::_InitChildNodes( const SInitChildNodesInfo& Info )
        {
            ...
            VKE_ASSERT( childNodeLevel < Info.maxLODCount, "" );
            {
                ...
                ChildOfChildInfo.vecExtents =
                    Math::CVector3( ChildOfChildInfo.vec4Extents );
                ChildOfChildInfo.boundingSphereRadius =
                    Info.boundingSphereRadius * 0.5f;
                ChildOfChildInfo.maxLODCount = Info.maxLODCount;
                {
                    VKE_PROFILE_SIMPLE2(
                        "create child nodes for parent" ); // 6 us
                                                           // Create child nodes
                                                           // for parent
                    for( uint8_t i = 0; i < 4; ++i )
                    {
                        ...
                        Math::CVector4::Mad( aVectors[ i ], Info.vec4Extents,
                                             Info.vec4ParentCenter,
                                             &vecChildCenter );
                        Node.AABB = Math::CAABB(
                            Math::CVector3{ vecChildCenter }, Info.vecExtents );
                ...
                if( childNodeLevel + 1 < Info.maxLODCount )
                {
                    ...
                    for( uint8_t i = 0; i < 4; ++i )
                    {
                        ...
                        ChildOfChildInfo.childNodeStartIndex =
                            _AcquireChildNodes();
        ...
        void CTerrainQuadTree::_Destroy() { m_vNodes.Destroy(); }
        void
        CTerrainQuadTree::_SetDrawDataForNode( CTerrainQuadTree::SNode* pInOut )
        {
            auto pPipeline =
                this->m_pTerrain->_GetPipelineForLOD( pInOut->Handle.level );
            VKE_ASSERT( pPipeline.IsValid(),
                        "Pipeline must not be null at this stage" );
            pInOut->DrawData.pPipeline = pPipeline;
            pInOut->DrawData.vecPosition.x =
                pInOut->AABB.Center.x - pInOut->AABB.Extents.x;
            pInOut->DrawData.vecPosition.y = pInOut->AABB.Center.y;
            pInOut->DrawData.vecPosition.z =
                pInOut->AABB.Center.z + pInOut->AABB.Extents.z;
            /*const auto& p = pInOut->DrawData.vecPosition;
            VKE_LOG(p.x << ", " << p.z);*/
        }
        Result
        CTerrainQuadTree::_CreateChildNodes( UNodeHandle hParent,
                                             const SCreateNodeData& NodeData,
                                             const uint8_t lodCount )
        {
            ...
            if( currLevel < lodCount )
            {
                ...
                ChildNodeData.vecExtents =
                    Math::CVector3( ChildNodeData.vec4Extents );
                ChildNodeData.boundingSphereRadius =
                    NodeData.boundingSphereRadius * 0.5f;
                ...
                for( uint8_t i = 0; i < 4; ++i )
                {
                    ...
                    Node.ahChildren[ 2 ].handle = UNDEFINED_U32;
                    Node.ahChildren[ 3 ].handle = UNDEFINED_U32;
                    Math::CVector4::Mad( aVectors[ i ], NodeData.vec4Extents,
                                         NodeData.vec4ParentCenter,
                                         &vecChildCenter );
                    Node.AABB = Math::CAABB( Math::CVector3{ vecChildCenter },
                                             NodeData.vecExtents );
                    ...
                    ahChildNodes[ i ] = Handle;
                    m_vAABBs[ Handle.index ] = Node.AABB;
                    m_vBoundingSpheres[ Handle.index ] = Math::CBoundingSphere(
                        Node.AABB.Center, Node.boundingSphereRadius );
        ...
        CTerrainQuadTree::CHILD_NODE_INDEX
        CTerrainQuadTree::_CalcNodeIndex( const Math::CVector4& vecParentCenter,
        ...
        void CTerrainQuadTree::_Update()
        {
            ...
            View.frustumWidth = pCamera->CalcFrustumWidth(
                1.0f ); // frustum width at 1.0f distance
            ...
            }
            {
                ...
                // const uint32_t nodeCount = m_RootNodeCount.x *
                // m_RootNodeCount.y; for (uint32_t i = 0; i < nodeCount; ++i)
                //{
                                     ...
                                     Math::CVector4* pOut )
        {
            ...
            vecDir.Normalize();
            Math::CVector4::Mad( vecDir, Math::CVector4( sphereRadius ),
                                 vecSphereCenter, pOut );
        }
        ...
        vke_force_inline uint32_t MapPositionTo1DArrayIndex(
            const Math::CVector3& vecPos, const uint32_t& tileSize,
            const uint32_t& terrainHalfSize, const uint32_t& elementCountInRow )
        {
            // (pos + terrain half size) / element size
            const uint32_t x =
                ( ( int32_t )vecPos.x + terrainHalfSize ) / tileSize;
            const uint32_t y =
                ( ( int32_t )-vecPos.z + terrainHalfSize ) / tileSize;
            uint32_t ret =
                Math::Map2DArrayIndexTo1DArrayIndex( x, y, elementCountInRow );
            return ret;
        }
        ...
        // mipmaps).
        vke_force_inline uint16_t CalcRenderTileSize(
            const uint8_t renderLOD, const uint16_t baseTileSize )
        {
            return Math::CalcPow2( renderLOD ) * baseTileSize;
        }
        void CTerrainQuadTree::_CalcErrorLODs( const SNode& CurrNode,
                                               ...
                                               const SViewData& View )
        {
            ...
            /*VKE_DBG_LOG( "" << indents[ hCurrNode.level ] << "l: " <<
               hCurrNode.level << " idx: " << hCurrNode.index << " d: " <<
               distance << " e: " << err << " c: " << AABB.Center.x << ", " <<
               AABB.Center.z << " p: " << vecPoint.x << ", " << vecPoint.z << "
               vp: " << CurrNode.DrawData.vecPosition.x << ", " <<
                ...
                // if( CurrNode.ahChildren[ 0 ].handle != UNDEFINED_U32 )
                {
                    ...
                    /*for( uint32_t i = 0; i < 4; ++i )
                    {
                        const auto hNode = CurrNode.ahChildren[ i ];
                        {
                            ...
                            CalcNearestSpherePoint( Math::CVector4(
                    Node.AABB.Center ), Node.boundingSphereRadius,
                                                    Math::CVector4(
                    View.vecPosition ), &vecPoint ); _CalcError( vecPoint,
                    hNode.level, View, &childErr, &childDistance );
                    ...
                    for( uint32_t i = 0; i < 4; ++i )
                    {
                        ...
                        const Math::CVector3 vecTmpPos2 =
                            AABB2.Center - AABB2.Extents;
            ...
            else // if( hCurrNode.level > 0 )
            {
                ...
                // Data.idx = MapPositionTo1DArrayIndex(
                // Data.DrawData.vecPosition, m_tileSize,
                //                                      m_terrainHalfSize,
                //                                      m_tileInRowCount );
                ///*VKE_DBG_LOG("" << indents[hCurrNode.level] << "l: " <<
                /// hCurrNode.level << " idx: " << hCurrNode.index <<
                //    " d: " << distance << " e: " << err <<
        ...
        void LoadPosition3( const Math::CVector4 aVecs[ 3 ], uint32_t idx,
                            Math::CVector3* pOut )
        {
        ...
        void LoadExtents3( const Math::CVector4 aVecs[ 2 ], uint32_t idx,
                           Math::CVector3* pOut )
        {
            pOut->x = aVecs[ CTerrainQuadTree::SNodeLevel::X ].floats[ idx ];
            pOut->y = aVecs[ CTerrainQuadTree::SNodeLevel::Y ].floats[ idx ];
        ...
        void LoadPosition4( const Math::CVector4 aVecs[ 3 ], uint32_t idx,
                            Math::CVector4* pOut )
        {
            pOut->x = aVecs[ CTerrainQuadTree::SNodeLevel::X ].floats[ idx ];
            pOut->y = aVecs[ CTerrainQuadTree::SNodeLevel::Y ].floats[ idx ];
        ...
        void LoadExtents4( const Math::CVector4 aVecs[ 2 ], uint32_t idx,
                           Math::CVector4* pOut )
        {
            pOut->x = aVecs[ CTerrainQuadTree::SNodeLevel::X ].floats[ idx ];
            pOut->y = aVecs[ CTerrainQuadTree::SNodeLevel::Y ].floats[ idx ];
        }
        void LoadAABB( const Math::CVector4 aCenters[ 3 ],
                       const Math::CVector4 aExtents[ 2 ], uint32_t idx,
                       Math::CAABB* pOut )
        {
            pOut->Center.x =
                aCenters[ CTerrainQuadTree::SNodeLevel::X ].floats[ idx ];
            pOut->Center.z =
                aCenters[ CTerrainQuadTree::SNodeLevel::Z ].floats[ idx ];
            pOut->Center.y =
                aCenters[ CTerrainQuadTree::SNodeLevel::Y ].floats[ idx ];
            pOut->Extents.x =
                aExtents[ CTerrainQuadTree::SNodeLevel::X ].floats[ idx ];
            pOut->Extents.y =
                aExtents[ CTerrainQuadTree::SNodeLevel::Y ].floats[ idx ];
            pOut->Extents.z = pOut->Extents.x;
        }
        void vke_force_inline CalcLenghts3( const Math::CVector4* aVectors,
                                            Math::CVector4* pOut )
        {
            Math::CVector4 aMuls[ 3 ], aAdds[ 2 ];
            // x*x, y*y, z*z
            {
                Math::CVector4::Mul( aVectors[ 0 ], aVectors[ 0 ],
                                     &aMuls[ 0 ] );
                Math::CVector4::Mul( aVectors[ 1 ], aVectors[ 1 ],
                                     &aMuls[ 1 ] );
                Math::CVector4::Mul( aVectors[ 2 ], aVectors[ 2 ],
                                     &aMuls[ 2 ] );
            }
            // x*x + y*y + z*z
            {
        ...
        void vke_force_inline CalcDistance3( const Math::CVector4* aV1,
                                             const Math::CVector4* aV2,
                                             Math::CVector4* pOut )
        {
...
#define VKE_LOAD_VEC4_XZY( _vec4 )                                             \
    Math::CVector4{ ( _vec4 ).x }, Math::CVector4{ ( _vec4 ).z },              \
        Math::CVector4                                                         \
    {                                                                          \
        ( _vec4 ).y                                                            \
    }
        void CalcScreenSpaceErrors( const Math::CVector4* aPoints,
                                    const float& worldSpaceError,
                                    const CTerrainQuadTree::SViewData& View,
                                    Math::CVector4* pvecErrorOut,
                                    Math::CVector4* pvecDistanceOut )
        {
            ...
            /// TODO: Cache this!
            const float k =
                View.screenWidth / ( 2.0f * std::tanf( View.halfFOV ) );
        ...
        void CalcErrors( const Math::CVector4* aPoints,
                         const float vertexDistance, const uint8_t nodeLevel,
                         const uint8_t lodCount,
                         const CTerrainQuadTree::SViewData& View,
                         Math::CVector4* pvecErrorOut,
                         Math::CVector4* pvecDistanceOut )
        {
            // VKE_PROFILE_SIMPLE();
            const float worldSpaceError =
                CalcWorldSpaceError( vertexDistance, nodeLevel, lodCount );
            CalcScreenSpaceErrors( aPoints, worldSpaceError, View, pvecErrorOut,
                                   pvecDistanceOut );
        }
        void CalcNearestSpherePoints( const Math::CVector4* aSphereCenters,
                                      const float sphereRadius,
                                      const Math::CVector4& vec4ViewPos,
                                      Math::CVector4* pOut )
        {
            // VKE_PROFILE_SIMPLE();
            ...
            const Math::CVector4 aViewPos[ 3 ] = {
                ...
                Math::CVector4{ vec4ViewPos.z }, Math::CVector4{ vec4ViewPos.y }
            };
            ...
            //
            //
            Math::CVector4::Sub( aViewPos[ 0 ], aSphereCenters[ 0 ],
                                 &aDirs[ 0 ] );
            Math::CVector4::Sub( aViewPos[ 1 ], aSphereCenters[ 1 ],
                                 &aDirs[ 1 ] );
            Math::CVector4::Sub( aViewPos[ 2 ], aSphereCenters[ 2 ],
                                 &aDirs[ 2 ] );
            // normalize dirs
            {
            ...
            Math::CVector4::Mad( aDirs[ 0 ], vecRadius, aSphereCenters[ 0 ],
                                 &pOut[ 0 ] );
            Math::CVector4::Mad( aDirs[ 1 ], vecRadius, aSphereCenters[ 1 ],
                                 &pOut[ 1 ] );
            Math::CVector4::Mad( aDirs[ 2 ], vecRadius, aSphereCenters[ 2 ],
                                 &pOut[ 2 ] );
        }
        uint32_t MapTileIndexToRootIndex( const uint32_t& tileIdx,
                                          const ExtentU16& RootNodeCount,
                                              ...
                                              const SViewData& View )
        {
            ...
            const Math::CVector4 vec4ViewPosition =
            ...
            float err, distance;
            CalcNearestSpherePoint( vec4Center, boundingSphereRadius,
                                    vec4ViewPosition, &vec4NearestPoint );
            _CalcError( vec4NearestPoint, Root.Handle.level, View, &err,
                        &distance );
            if( hasChildNodes && err > 60 )
            {
                const SNodeLevel& ChildNodes =
                    m_vChildNodeLevels[ Root.childLevelIndex ];
        ...
        {
            const bool hasChildNodes =
                ChildNodes.aChildLevelIndices[ 0 ] != UNDEFINED_U32;
            // const float boundingSphereRadius =
            ...
            Math::CVector4 vec4ChildCenter, vec4ChildExtents, vecErrors,
                vecDistances;
            ...
            }
            {
                ...
                CalcNearestSpherePoints( ChildNodes.aAABBCenters,
                                         boundingSphereRadius, vec4ViewPosition,
                                         aNearestPoints );
                CalcErrors( aNearestPoints, m_Desc.vertexDistance,
                            ChildNodes.level, m_Desc.lodCount, View, &vecErrors,
                            &vecDistances );
            }
            for( uint32_t i = 0; i < 4; ++i )
            {
                ...
                aChildNodeStates[ i ] =
                    ( 1 + ( hasChildNodes && vecErrors.floats[ i ] > 60 ) ) *
                    visible;
                if( aChildNodeStates[ i ] == 1 )
                {
                    SLODInfo Info;
                    LoadPosition4( ChildNodes.aAABBCenters, i,
                                   &Info.vec4Center );
                    LoadExtents3( ChildNodes.aAABBExtents, i,
                                  &Info.nodeExtents );
                    Info.nodeLevel = ChildNodes.level;
                    Info.bindingIndex = ChildNodes.DrawData.bindingIndex;
                    Info.vec3RootPosition =
                        m_vNodes[ ChildNodes.rootNodeIndex ].vec3Position;
#if VKE_SCENE_DEBUG
            ...
            for( uint32_t i = 0; i < 4; ++i )
            {
                ...
                if( aChildNodeStates[ i ] == 2 )
                {
                    const SNodeLevel& Level =
                        m_vChildNodeLevels[ ChildNodes
                                                .aChildLevelIndices[ i ] ];
                    _CalcLODsSIMD( Level, Root, View );
                }
            }
        }
        void CTerrainQuadTree::_CalcDistanceLODs( const SNode& CurrNode,
                                                  const uint32_t& textureIdx,
                                                  const SViewData& View )
        {
            const auto hCurrNode = CurrNode.Handle;
            ...
            Math::CVector4 vecPoint;
            CalcNearestSpherePoint(
                Math::CVector4( AABB.Center ), CurrNode.boundingSphereRadius,
                Math::CVector4( View.vecPosition ), &vecPoint );
            //_CalcError( vecPoint, hCurrNode.level, View, &err, &distance );
            // float distance = _CalcDistanceToCenter( AABB.Center, View );
            float distance = Math::CVector4::Distance(
                vecPoint, Math::CVector4( View.vecPosition ) );
            // float childErr, childDistance;
            static cstr_t indents[] = {
                "",
                " ",
            ...
            const uint32_t lod = Math::Min(
                ( ( uint32_t )distance / m_tileSize - 1 ), highestLod );
        ...
        void CTerrainQuadTree::_SetLODMap( const SLODData& Data )
        {
            ...
            Math::Map1DarrayIndexTo2DArrayIndex(
                Data.idx, m_tileInRowCount, m_tileInRowCount, &currX, &currY );
            ...
            for( uint32_t y = 0; y < tileRowCount; ++y )
            {
                const uint32_t leftIdx = Math::Map2DArrayIndexTo1DArrayIndex(
                    currX, y + currY, m_tileInRowCount );
                const uint32_t rightIdx = Math::Map2DArrayIndexTo1DArrayIndex(
                    rightColIdx, y + currY, m_tileInRowCount );
                m_vLODMap[ leftIdx ] = lod;
                m_vLODMap[ rightIdx ] = lod;
            }
            for( uint32_t x = 0; x < tileColCount; ++x )
            {
                const uint32_t topIdx = Math::Map2DArrayIndexTo1DArrayIndex(
                    currX + x, currY, m_tileInRowCount );
                const uint32_t bottomIdx = Math::Map2DArrayIndexTo1DArrayIndex(
                    currX + x, bottomRowIdx, m_tileInRowCount );
            ...
            for( uint32_t y = 0; y < tileColCount; ++y )
            {
                ...
                {
                    uint32_t idx = Math::Map2DArrayIndexTo1DArrayIndex(
                        x + currX, y + currY, m_tileInRowCount );
        ...
        void CTerrainQuadTree::_SetStitches()
        {
            for( uint32_t i = 0; i < m_vLODData.GetCount(); ++i )
            {
                ...
                uint32_t tileRowCount =
                    Math::CalcPow2( Data.lod ); // number of tiles for this node
                auto pos = Data.DrawData.vecPosition;
                // x, y are the left top corner in the map
                uint32_t x, y;
                Math::Map1DarrayIndexTo2DArrayIndex( Data.idx, m_tileInRowCount,
                                                     m_tileInRowCount, &x, &y );
                ...
                if( x > 0 )
                {
                    uint32_t leftIdx = Math::Map2DArrayIndexTo1DArrayIndex(
                        x - 1, y, m_tileInRowCount );
                    leftLod = m_vLODMap[ leftIdx ];
                }
                if( y > 0 )
                {
                    uint32_t topIdx = Math::Map2DArrayIndexTo1DArrayIndex(
                        x, y - 1, m_tileInRowCount );
                    topLod = m_vLODMap[ topIdx ];
                }
                if( x + tileRowCount < m_tileInRowCount )
                {
                    uint32_t rightIdx = Math::Map2DArrayIndexTo1DArrayIndex(
                        x + tileRowCount, y, m_tileInRowCount );
                    rightLod = m_vLODMap[ rightIdx ];
                }
                if( y + tileRowCount < m_tileInRowCount )
                {
                    uint32_t bottomIdx = Math::Map2DArrayIndexTo1DArrayIndex(
                        x, y + tileRowCount, m_tileInRowCount );
                    bottomLod = m_vLODMap[ bottomIdx ];
                }
                if( Data.lod < leftLod )
                {
                    Data.DrawData.leftVertexDiff = ( uint8_t )Math::CalcPow2(
                        leftLod - ( uint8_t )Data.lod );
                }
                if( Data.lod < rightLod )
                {
                    Data.DrawData.rightVertexDiff = ( uint8_t )Math::CalcPow2(
                        rightLod - ( uint8_t )Data.lod );
                }
                if( Data.lod < topLod )
                {
                    Data.DrawData.topVertexDiff = ( uint8_t )Math::CalcPow2(
                        topLod - ( uint8_t )Data.lod );
                }
                if( Data.lod < bottomLod )
                {
                    Data.DrawData.bottomVertexDiff = ( uint8_t )Math::CalcPow2(
                        bottomLod - ( uint8_t )Data.lod );
                    ;
                }
            }
        ...
        void CTerrainQuadTree::_SetLODData( const SLODInfo& Info,
                                            SLODData* pOut ) const
        {
            const uint8_t highestLod = ( uint8_t )( m_Desc.lodCount - 1 );
            ...
            ImageSize Offset = m_pTerrain->m_Desc.HeightmapOffset;
            {
                ...
                CalcNodePosition( Info.vec4Center, Info.nodeExtents,
                                  &DrawData.vecPosition );
            }
            pOut->idx = MapPositionTo1DArrayIndex(
                DrawData.vecPosition, m_tileSize, m_terrainHalfSize,
                m_tileInRowCount );
        ...
        float CalcWorldSpaceError( const float vertexDistance,
                                   ...
                                   const uint8_t levelCount )
        {
            const uint32_t levelDistance =
                ( 1u << ( levelCount - nodeLevel - 1 ) );
            const float ret = vertexDistance * levelDistance;
            return ret;
        }
        void CalcScreenSpaceError( const Math::CVector4& vecPoint,
                                   const float& worldSpaceError,
                                   const CTerrainQuadTree::SViewData& View,
                                   float* pErrOut, float* pDistanceOut )
        {
            const float distance = Math::CVector4::Distance(
                Math::CVector4( View.vecPosition ), vecPoint );
        ...
        void CTerrainQuadTree::_CalcError( const Math::CVector4& vecPoint,
                                           const uint8_t nodeLevel,
                                           const SViewData& View,
                                           float* pErrOut,
                                           float* pDistanceOut ) const
        {
            const float worldSpaceError = CalcWorldSpaceError(
                m_Desc.vertexDistance, nodeLevel, m_Desc.lodCount );
            CalcScreenSpaceError( vecPoint, worldSpaceError, View, pErrOut,
                                  pDistanceOut );
        }
        float
        CTerrainQuadTree::_CalcDistanceToCenter( const Math::CVector3& vecPoint,
                                                 const SViewData& View )
        {
            return Math::CVector3::Distance( vecPoint, ( View.vecPosition ) );
        }
        CTerrainQuadTree::UNodeHandle
        CTerrainQuadTree::_FindNode( const SNode& Node,
        ...
        void IntersectsSIMD( const Math::CVector4 aSphereCenters[ 3 ],
                             const Math::CVector4& v4Radius,
                             ...
                             Math::CVector4* pOut )
        {
            ...
            plx0x1x2x3 = { aPlanes[ 0 ].x, aPlanes[ 1 ].x, aPlanes[ 2 ].x,
                           aPlanes[ 3 ].x };
            plx4x5x4x5 = { aPlanes[ 4 ].x, aPlanes[ 5 ].x, aPlanes[ 4 ].x,
                           aPlanes[ 5 ].x };
            ply0y1y2y3 = { aPlanes[ 0 ].y, aPlanes[ 1 ].y, aPlanes[ 2 ].y,
                           aPlanes[ 3 ].y };
            ply4y5y4y5 = { aPlanes[ 4 ].y, aPlanes[ 5 ].y, aPlanes[ 4 ].y,
                           aPlanes[ 5 ].y };
            plz0z1z2z3 = { aPlanes[ 0 ].z, aPlanes[ 1 ].z, aPlanes[ 2 ].z,
                           aPlanes[ 3 ].z };
            plz4z5z4z5 = { aPlanes[ 4 ].z, aPlanes[ 5 ].z, aPlanes[ 4 ].z,
                           aPlanes[ 5 ].z };
            plw0w1w2w3 = { aPlanes[ 0 ].w, aPlanes[ 1 ].w, aPlanes[ 2 ].w,
                           aPlanes[ 3 ].w };
            plw4w5w4w5 = { aPlanes[ 4 ].w, aPlanes[ 5 ].w, aPlanes[ 4 ].w,
                           aPlanes[ 5 ].w };
            auto mulX =
                DirectX::XMVectorMultiply( plx0x1x2x3, centerX._Native );
            auto mulY =
                DirectX::XMVectorMultiply( ply0y1y2y3, centerY._Native );
            auto mulZ =
                DirectX::XMVectorMultiply( plz0z1z2z3, centerZ._Native );
            ...
            auto le = DirectX::XMVectorLessOrEqual(
                addXYZW, DirectX::XMVectorNegate( v4Radius._Native ) );
            auto ci = DirectX::XMVectorLessOrEqual(
                v4Radius._Native, Math::CVector4::ZERO._Native );
        ...
        void Intersects( const Math::CVector4 aSphereCenters[ 3 ],
                         const Math::CVector4& v4Radius,
        ...
        void CTerrainQuadTree::_FrustumCullChildNodes( const SViewData& View )
        {
            if constexpr( !DISABLE_FRUSTUM_CULLING )
            {
                for( uint32_t i = 0; i < m_vVisibleRootNodes.GetCount(); ++i )
                {
                    ...
                    SNodeLevel& ChildNodes =
                        m_vChildNodeLevels[ Root.childLevelIndex ];
                    _FrustumCullChildNodes( View, ChildNodes,
                                            &ChildNodes.vecVisibility );
                }
            }
        }
        void CTerrainQuadTree::_FrustumCullChildNodes( const SViewData& View,
                                                       ...
                                                       Math::CVector4* pOut )
        {
            if constexpr( !DISABLE_FRUSTUM_CULLING )
            {
                Intersects( Level.aAABBCenters,
                            Math::CVector4( Level.boundingSphereRadius ),
                            View.Frustum, pOut );
            }
            else
            {
                pOut->ints[ 0 ] = pOut->ints[ 1 ] = pOut->ints[ 2 ] =
                    pOut->ints[ 3 ] = 1; // set visible always to true
            }
        }
        void
        CTerrainQuadTree::_BoundingSphereFrustumCull( const SViewData& View )
        {
        ...
        void CTerrainQuadTree::_FrustumCull( const SViewData& View )
        {
            ...
            {
                _BoundingSphereFrustumCullNode( m_vVisibleRootNodeHandles[ i ],
                                                Frustum );
            }
        }
        void CTerrainQuadTree::_BoundingSphereFrustumCullNode(
            const UNodeHandle& hNode, const Math::CFrustum& Frustum )
        {
            const Math::CBoundingSphere& Sphere =
                m_vBoundingSpheres[ hNode.index ];
            const bool isVisible = Frustum.Intersects( Sphere );
            ...
            if( isVisible )
            {
                ...
                if( ChildNodes.aHandles[ 0 ].handle != UNDEFINED_U32 )
                {
                    for( uint32_t i = 0; i < 4; ++i )
                    {
                        _BoundingSphereFrustumCullNode(
                            ChildNodes.aHandles[ i ], Frustum );
                    }
                }
            }
        }
        void
        CTerrainQuadTree::_UpdateRootNode( const uint32_t& rootNodeIndex,
