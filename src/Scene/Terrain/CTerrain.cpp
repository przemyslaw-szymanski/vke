...
namespace VKE
{
    namespace Scene
    {
        void CTerrain::CalcTextureCount( const STerrainDesc& Desc, ExtentU32* pOut )
        {
        ...
        Result CTerrain::_CreateDummyResources(RenderSystem::CDeviceContext* pCtx)
        {
            Result ret = VKE_FAIL;
            {
                ...
                {
                    {
                        ...
                        // Set normal texview dummy in case when there is no normal textures
...
#define VKE_LOAD_VEC4_XZY(_vec4) Math::CVector4{(_vec4).x}, Math::CVector4{(_vec4).z}, Math::CVector4{(_vec4).y}
            ...
            const SNode& Root, const SViewData& View)
        {
            ...
            const Math::CVector4 vec4ViewPosition = Math::CVector4( View.vecPosition );
            uint8_t aChildNodeStates[4]; // 0 - do nothing, 1 - add lod, 2 - calc lod for next level
            ...
            for (uint32_t i = 0; i < 4; ++i)
            {
                ...
                aChildNodeStates[i] = ( 1 + ( hasChildNodes && vecErrors.floats[i] > 60 ) ) * visible;
