...
namespace VKE
{
    namespace Input
    {
                    ...
                    for( uint32_t i = 0; i < count; ++i )
                    {
                        ...
                        switch( Rdi.dwType )
                        {
                            ...
                            case RIM_TYPEKEYBOARD: {
                                ...
                                Device.Keyboard.funcKeyCount =
                                    static_cast< uint16_t >( Rdi.keyboard.dwNumberOfFunctionKeys );
        ...
        Result CInputSystem::_Create( const SInputSystemDesc& Desc )
        {
            ...
            const ::RAWINPUTDEVICE aDevices[ 2 ] = { m_Keyboard, m_Mouse };
            ...
            if( VKE_SUCCEEDED( _QueryDevices() ) )
            {
                ...
                auto res = ::RegisterRawInputDevices(
                    vRawDevices.GetData(), vRawDevices.GetCount(), sizeof( ::RAWINPUTDEVICE ) );
                if( res != FALSE )
                {
                    ret = VKE_OK;
                }
