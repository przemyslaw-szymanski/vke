    ...
    namespace RenderSystem
    {
        ...
        struct VKE_API SColor
        {
            ...
            explicit SColor( uint32_t v )
                ...
                , g( static_cast<float>( v ) )
                , b( static_cast<float>( v ) )
        ...
        struct VKE_API SDepthStencilValue
        {
            ...
            SDepthStencilValue( float d, uint32_t s )
                : depth{ d }
                , stencil{ s }
            {
            }
        ...
        struct VKE_API SClearValue
        {
            ...
            SClearValue( const SClearValue& V )
                : Color( V.Color )
            {
            }
            SClearValue( const SColor& C )
                : Color{ C }
            {
            }
            SClearValue( const SDepthStencilValue& DS )
                : DepthStencil{ DS }
            {
            }
            ...
            SClearValue( float d, uint32_t s )
                : DepthStencil( d, s )
            {
            }
        };
        ...
        struct SUpdateBindingsHelper
        {
            ...
            void AddBinding(
                uint8_t binding, const SamplerHandle* ahSamplers, const TextureViewHandle* ahTexViews,
            ...
            void AddBinding(
                uint8_t binding, const uint32_t offset, const uint32_t range, const BufferHandle& hBuffer,
                BINDING_TYPE type )
            {
        ...
        struct SFramebufferDesc
        {
            ...
            TextureSize      Size;
            AttachmentArray  vNativeAPIAttachments;
            RenderPassHandle hRenderPass;
            VKE_RENDER_SYSTEM_DEBUG_NAME;
        };
        struct TextureTypes
        ...
        {
            NativeAPI::TextureView       hNativeAPIView = NativeAPI::Null;
        ...
        struct SBeginRenderPassInfo
        {
            ...
            ClearValueArray        vNativeAPIClearValues;
            NativeAPI::Framebuffer hNativeAPIFramebuffer;
            NativeAPI::RenderPass  hNativeAPIRenderPass;
            Rect2DI32              RenderArea;
        };
        ...
        struct SBeginRenderPassInfo2
        {
            ...
            static vke_force_inline hash_t CalcHash( const SBeginRenderPassInfo2& Info )
            {
                ...
                Hash.Combine(
                    ...
                    Info.DepthRenderTargetInfo.ClearColor.DepthStencil.stencil, Info.DepthRenderTargetInfo.hNativeAPIView,
                    Info.DepthRenderTargetInfo.renderPassOp, Info.DepthRenderTargetInfo.state,
                    Info.StencilRenderTargetInfo.ClearColor.DepthStencil.stencil, Info.StencilRenderTargetInfo.hNativeAPIView,
                ...
                for( uint32_t i = 0; i < Info.vColorRenderTargetInfos.GetCount(); ++i )
                {
                    ...
                    Hash.Combine(
                        RT.ClearColor.Color.r, RT.ClearColor.Color.g, RT.ClearColor.Color.g, RT.ClearColor.Color.a,
                        RT.hNativeAPIView, RT.renderPassOp, RT.state );
                }
                return Hash.value;
            }
        };
        ...
        {
            NativeAPI::CommandBuffer    hNativeAPICommandBuffer;
        ...
        struct SPipelineDesc
        {
            ...
            struct SInputLayout
            {
                struct SVertexAttribute
                {
                    ...
                    SVertexAttribute( DEFAULT_CTOR_INIT )
                        : pName( "" )
                        , format{ Formats::R32G32B32_SFLOAT }
                        , vertexBufferBindingIndex{ 0 }
                        , location{ 0 }
                        , offset{ 0 }
                        , stride{ 3 * 4 }
                        , inputRate{ VertexInputRates::VERTEX }
                    {
                    }
                    SVertexAttribute( cstr_t name, const FORMAT& fmt, uint16_t bindingLocation )
                        ...
                        , offset{ 0 }
                        , stride{ CalcFormatSize( fmt ) }
                        , inputRate{ VertexInputRates::VERTEX }
                    {
                    }
                    SVertexAttribute(
                        cstr_t name, const FORMAT& fmt, const uint16_t& bufferBindingIdx,
                        const uint16_t& bindingLocation, const uint16_t& off, const uint16_t& strd,
                        const VERTEX_INPUT_RATE& rate )
                        : pName{ name }
                        , format{ fmt }
                        , vertexBufferBindingIndex{ bufferBindingIdx }
                        , location{ bindingLocation }
                        , offset{ off }
                        , stride{ strd }
                        , inputRate{ rate }
                ...
                using SVertexAttributeArray = Utils::TCDynamicArray<
                    SVertexAttribute, Config::RenderSystem::Pipeline::MAX_VERTEX_ATTRIBUTE_COUNT>;
            ...
            NativeAPI::PipelineLayout hNativeAPILayout                = NativeAPI::Null;
            RenderPassHandle          hRenderPass               = INVALID_HANDLE;
            NativeAPI::RenderPass     hNativeAPIRenderPass            = NativeAPI::Null;
            NativeAPI::Pipeline       hNativeAPIParent                = NativeAPI::Null;
        ...
        struct SNativeAPIGetBackBufferInfo
        {
            uint64_t            waitTimeout = UINT64_MAX;
            NativeAPI::Fence    hSignalGPUFence;
            NativeAPI::CPUFence hSignalCPUFence = NativeAPI::Null;
        };
        struct SNativeAPILoadInfo
        {
            SAPIAppInfo AppInfo;
        ...
        struct SSubmitInfo
        {
            NativeAPI::Fence*         pWaitFences        = nullptr;
            uint64_t*                 pWaitFenceValues   = nullptr;
            NativeAPI::Fence*         pSignalFences      = nullptr;
            uint64_t*                 pSignalFenceValues = nullptr;
            NativeAPI::CommandBuffer* pNativeAPICommandBuffers = nullptr;
            NativeAPI::CPUFence       hNativeAPIFence          = NativeAPI::Null;
            NativeAPI::Queue          hNativeAPIQueue          = NativeAPI::Null;
            uint16_t                  signalFenceCount   = 0;
            uint16_t                  waitFenceCount     = 0;
            uint16_t                  commandBufferCount = 0;
        };
        struct SPresentInfo
        {
            uint32_t imageIndex;
            // NativeAPI::SwapChain    hNativeAPISwapChain;
            CSwapChain*      pSwapChain;
            NativeAPI::Fence hNativeAPIWaitSemaphore = NativeAPI::Null;
        };
        struct SPresentData
        {
            using UintArray      = Utils::TCDynamicArray<uint32_t, 8>;
            using SemaphoreArray = Utils::TCDynamicArray<NativeAPI::Fence, 8>;
        ...
        {
            NativeAPI::Memory hNativeAPIMemory;
            uint32_t          sizeLeft;
        ...
        struct SBindMemoryInfo
        {
            ...
            NativeAPI::Memory  hNativeAPIMemory  = NativeAPI::Null;
        ...
        struct SBindNativeAPIDescriptorSetsInfo
        {
            NativeAPI::CommandBuffer        hNativeAPICommandBuffer;
            const NativeAPI::DescriptorSet* aNativeAPISetHandles;
            const uint32_t*                 aDynamicOffsets = nullptr;
            NativeAPI::PipelineLayout       hNativeAPIPipelineLayout;
            uint16_t                        firstSet;
            uint16_t                        setCount;
            uint16_t                        dynamicOffsetCount = 0;
            PIPELINE_TYPE                   pipelineType;
        };
        struct SNativeAPISwapChainDesc
        {
            TextureSize    Size             = { 800, 600 };
            uint32_t       queueFamilyIndex = 0;
            COLOR_SPACE    colorSpace       = ColorSpaces::SRGB;
        ...
        {
            NativeAPI::CommandBufferPool hNativeAPIPool;
            uint32_t                     count;
        ...
        {
            NativeAPI::CommandBufferPool hNativeAPIPool;
            NativeAPI::CommandBuffer*    pNativeAPICommandBuffers;
            uint32_t                     count;
        };
        ...
        struct SFrameGraphNodeDesc
        {
            ...
            cstr_t           pThread        = "Main";
            cstr_t           pCommandBufferName = "Main";
            cstr_t           pFenceName         = "Main";
            CONTEXT_TYPE     contextType    = ContextTypes::GENERAL;
            RENDER_PASS_SIZE size           = RenderPassSizes::_1_1;
            uint32_t         executeIndex   = INVALID_POSITION;
            TextureDescArray vRenderTargets;
            bool             signalEndFrameFence = false;
        };
        ...
        struct SFence
        {
            NativeAPI::Fence      hNative = NativeAPI::Null;
            NativeAPI::FenceValue value  = INVALID_POSITION;
        };

        struct SExecuteBatch
        {
            using FenceArray = Utils::TCDynamicArray< SFence, 4 >;
            static const uint32_t DEFAULT_CMD_BUFFER_COUNT = 32;
            using ExecuteBatchArray                        = Utils::TCDynamicArray<SExecuteBatch*, 4>;
            using SemaphoreArray                           = Utils::TCDynamicArray<NativeAPI::Fence, 8>;
            using UintArray                                 = Utils::TCDynamicArray<NativeAPI::FenceValue, 8 >;
            VKE_RENDER_SYSTEM_DEBUG_NAME;
            CContextBase*                                                    pContext        = nullptr;
            FenceArray                                                       vSignalFences;
            NativeAPI::CPUFence                                              hSignalCPUFence = NativeAPI::Null;
            SemaphoreArray                                                   vNativeAPIWaitGPUFences;
            UintArray                                                        vWaitFenceValues;
            ...
            void                         AddDependency( SExecuteBatch** ppBatch )
            {
                ...
                vNativeAPIWaitGPUFences.PushBack( pBatch->vSignalFences[0].hNative );
                vWaitFenceValues.PushBack( pBatch->vSignalFences[ 0 ].value );
...
#define VKE_ADD_NATIVE_API_OBJECT( _type )                                                                                    \
  protected:                                                                                                           \
    _type m_hNativeAPIObject = NativeAPI::Null;                                                                              \
                                                                                                                       \
  public:                                                                                                              \
    vke_force_inline const _type& GetNativeAPIObject() const                                                                 \
    {                                                                                                                  \
        return m_hNativeAPIObject;                                                                                           \
    }
    } // namespace RenderSystem
