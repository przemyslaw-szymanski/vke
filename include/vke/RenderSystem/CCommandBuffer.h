...
#define VKE_ENABLE_SIMPLE_COMMAND_BUFFER 1
    ...
    namespace RenderSystem
    {
        ...
        class VKE_API CCommandBuffer
        {
            ...
            VKE_ADD_DDI_OBJECT( NativeAPI::CommandBuffer );

            using States = CommandBufferStates;

        public:
            using DescSetArray      = Utils::TCDynamicArray< DescriptorSetHandle >;
            using DDIDescSetArray   = Utils::TCDynamicArray< NativeAPI::DescriptorSet >;
            using DDISemaphoreArray = Utils::TCDynamicArray< NativeAPI::GPUFence, 8 >;
        ...
        public:
            ...
            bool IsExecuted();
            void AddWaitOnSemaphore( const NativeAPI::GPUFence& hDDISemaphore );

            // RenderPassRefPtr        GetCurrentRenderPass() const { return m_pCurrentRenderPass; }
            // const NativeAPI::RenderPass&    GetCurrentDDIRenderPass() const { return m_hDDICurrentRenderPass; }
            ...
            COMMAND_BUFFER_STATE GetState() const
            {
                return m_state;
            }
            ...
            {
                return static_cast< uint8_t >( m_currBackBufferIdx );
            }
            ...
            // Commands
            void DrawFast( const uint32_t& vertexCount, const uint32_t& instanceCount, const uint32_t& firstVertex,
                           const uint32_t& firstInstance );
            void DrawIndexedFast( const SDrawParams& Params );

            void DrawFast( const uint32_t& vertexCount )
            {
                DrawFast( vertexCount, 1, 0, 0 );
            }

            void DrawWithCheck( const uint32_t& vertexCount, const uint32_t& instanceCount, const uint32_t& firstVertex,
                                const uint32_t& firstInstance );
            void DrawIndexedWithCheck( const SDrawParams& Params );

            void DrawWithCheck( const uint32_t& vertexCount )
            {
                DrawWithCheck( vertexCount, 1, 0, 0 );
            }
            ...
            {
                Draw< CheckState >( vertexCount, 1, 0, 0 );
            }

            template< CHECK_STATUS CheckState = DO_NOT_CHECK >
            void DrawIndexed( const uint32_t& indexCount )
            {
                DrawIndexed< CheckState >( indexCount, 1, 0, 0, 0 );
            }
            ...
            void SetEvent( const NativeAPI::Event& hDDIEvent, const PIPELINE_STAGES& stages );
            void SetEvent( const EventHandle& hEvent, const PIPELINE_STAGES& stages );
            void ResetEvent( const NativeAPI::Event& hDDIEvent, const PIPELINE_STAGES& stages );
            ...
            const HandleArray& GetStagingBufferAllocations() const
            {
                return m_vStagingBufferAllocations;
            }

            void ClearStagingBufferAllocations()
            {
                m_vStagingBufferAllocations.ClearFull();
            }

            handle_t GetLastUsedStagingBufferAllocation() const;
            void     UpdateStagingBufferAllocation( const handle_t& hStagingBuffer );

            void AddStagingBufferAllocation( const handle_t& hStagingBuffer )
            {
            ...
            void SignalGPUFence()
            {
                m_executeFlags |= ExecuteCommandBufferFlags::SIGNAL_GPU_FENCE;
            }
            ...
            const SCommandBufferState& GetCurrentState() const
            {
        ...
        protected:
            ...
            // void    _SetCPUSyncObject(const NativeAPI::CPUFence& hDDIFence) { m_hDDIFence = hDDIFence; }
            // void    _SetGPUSyncObject(NativeAPI::GPUFence hApi) { m_hApiGPUSyncObject = hApi; }
        ...
        protected:
            ...
            NativeAPI::PipelineLayout m_hDDILastUsedLayout = NativeAPI::Null;
            ...
            // NativeAPI::RenderPass               m_hDDICurrentRenderPass = NativeAPI::Null;
            NativeAPI::CPUFence          m_hApiCpuFence      = NativeAPI::Null;
            NativeAPI::GPUFence          m_hApiGpuFence      = NativeAPI::Null;
            NativeAPI::CommandBufferPool m_hDDICmdBufferPool = NativeAPI::Null;
