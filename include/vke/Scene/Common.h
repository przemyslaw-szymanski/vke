...
protected:                                                                                                             \
    VKE_SCENE_DEBUG_NAME_TYPE VKE_SCNEE_DEBUG_VAR_NAME
        ...
        struct STerrainDesc
        {
            ...
            using DDIRenderPassArray = Utils::TCDynamicArray< RenderSystem::NativeAPI::RenderPass >;
            ...
            uint32_t size = 0;
            /// Min and max tile sizes (width/depth). Min indicates the smallest (highest LOD) tile while Max is the
            /// root. A terrain is a grid of roots (max tile sizes). Min and max will be resized to nearest pow(2).
