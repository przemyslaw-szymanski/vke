...
namespace VKE
{
    ...
    namespace Scene
    {
        ...
        class VKE_API CScene : public Core::CObject
        {
            ...
            struct SDebugView
            {
                ...
                struct SInstancing
                {
                    ...
                    RenderSystem::NativeTypes::RenderPass hDDIRenderPass = RenderSystem::NativeTypes::Null;
            ...
            struct SDrawData
            {
                ...
                void Update( const uint32_t idx, bool isVisible )
                {
                    vVisibles[ idx ] = isVisible;
                }
                ...
                void Update( const uint32_t idx, const Math::CAABB& AABB )
                {
                    vAABBs[ idx ] = AABB;
                }

                void Update( const uint32_t idx, const Math::CMatrix4x4& Matrix )
                {
                    vTransforms[ idx ] = Matrix;
                }

                uint32_t GetCount() const
                {
                    return vVisibles.GetCount();
                }

                Math::CBoundingSphere& GetBoundingSphere( const uint32_t idx )
                {
                    return vBoundingSpheres[ idx ];
                }

                Math::CAABB& GetAABB( const uint32_t idx )
                {
                    return vAABBs[ idx ];
                }

                Math::CMatrix4x4& GetTransfrom( const uint32_t idx )
                {
                    return vTransforms[ idx ];
                }

                // UObjectBits& GetBits( const uint32_t idx ) { return vVisibles[idx]; }
                bool& GetVisible( const uint32_t idx )
                {
                    return vVisibles[ idx ];
                }
            };
        ...
        public:
            ...
            handle_t                  AddObject( RenderSystem::CommandBufferPtr, RenderSystem::DrawcallPtr pDrawcall,
                                                 const SDrawcallDataInfo& Info );
            void                      AddObject( const CModel* );
            CameraPtr                 CreateCamera( const SCameraDesc& );

            void SetViewCamera( CameraPtr pCamera )
            {
                m_pViewCamera = pCamera;
            }

            CameraPtr GetViewCamera() const
            {
            ...
            void UpdateDrawcallAABB( RenderSystem::CommandBufferPtr, const handle_t& hDrawcall,
            ...
            RenderSystem::CDeviceContext* GetDeviceContext() const
            {
                return m_pDeviceCtx;
            }
        ...
        protected:
            ...
            RenderSystem::DescriptorSetHandle
                m_ahBindings[ Config::RenderSystem::SwapChain::MAX_BACK_BUFFER_COUNT + 1 ];
