...
namespace VKE::Scene
{
    ...
    protected:
        ...
        void _CalcError( const Math::CVector4& vecPoint, const uint8_t nodeLevel, const SViewData& View, float* pErrOut,
        ...
        void        _GetAABBHeightSIMD( const SNodeLevel&, Math::CVector4* pvecMinHeightOut,
                                        Math::CVector4* pvecMaxHeightOut );
...
{
    struct STerrainTileDesc
    {
        STerrainSubTileDesc* pSubTiles;
    ...
    class CQuadTree
    {
        ...
        using NodeHandle = MemoryAllocator::SHandle;
        ...
        {
            ExtentF32  Size;
            NodeHandle aChildren[ 4 ] = { MemoryAllocator::INVALID_HANDLE };
            NodeHandle hData          = MemoryAllocator::INVALID_HANDLE;
            /// <summary>
            /// top-left, top-right, bottom-left, bottom-right
            /// it is used to calculate top left corner position of AABB
            /// </summary>
            uint8_t childIndex : 2;

            void Init()
            {
                hData          = MemoryAllocator::INVALID_HANDLE;
                aChildren[ 0 ] = aChildren[ 1 ] = aChildren[ 2 ] = aChildren[ 3 ] = MemoryAllocator::INVALID_HANDLE;
            }
        };

        struct SRootData
        {
            ExtentI32  TopLeftCornerPosition = { 0, 0 };
            uint32_t   size                  = 0;
            NodeHandle hNode;
        };

        using NodeBuffer     = Utils::TCDynamicArray< SNode, 1 >;
        using NodeDataBuffer = Utils::TCDynamicArray< STileData, 1 >;

    public:
        CQuadTree( const CTerrain* );
        ~CQuadTree();

        Result AddTerrainTile( const STerrainTileDesc& Desc );

    protected:
        Result     _Create();
        void       _Destroy();
        Result     _Resize( const ExtentI32& NewPos, uint32_t newSize );
        NodeHandle _CreateNode( ExtentI32 ParentPos, uint32_t parentSize, NodeHandle hParent, const ExtentI32& ChildPos,
                                uint8_t maxDepth );
        void       _CreateNode( const SNodeDesc& Desc );
        Result     _AllocateNode( NodeHandle* pNode );

        vke_force_inline SNode* _GetNode( NodeHandle handle )
        {
            return reinterpret_cast< SNode* >( m_NodeAllocator.Get( handle ) );
        }

        vke_force_inline STileData* _GetNodeData( NodeHandle handle )
        {
            return reinterpret_cast< STileData* >( m_NodeDataAllocator.Get( handle ) );
        }

    protected:
        STerrainDesc m_TerrainDesc;
        /// <summary>
        /// Every node has its own unique index in the buffer/array
        /// This index corresponds to NodeChildren and NodeData as well.
        /// </summary>
        MemoryAllocator m_NodeAllocator;
        MemoryAllocator m_NodeDataAllocator;
        SRootData       m_RootData;
