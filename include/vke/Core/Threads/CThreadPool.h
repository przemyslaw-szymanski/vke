...
namespace VKE
{
    ...
    namespace Threads
    {
        ...
        public:
            ...
            Result AddTask( ThreadUsages usages, cstr_t pDbgText, TaskFunction& Func, ArgsT&&... );
            template< typename... ArgsT >
            Result AddWorkerThreadTask( WorkerID workerIdx, ThreadUsages usages, cstr_t pDbgText, TaskFunction& Func,
                                        ArgsT&&... );

            // std::thread::id AddConstantTask( THREAD_USAGES usages, THREAD_TYPE_INDEX index, Threads::ITask*
            // pTask,TaskState state ); std::thread::id AddConstantTask( WorkerID wokerId, void* pData, TaskFunction2&&
            // Func ); std::thread::id AddConstantTask( WorkerID workerId, Threads::ITask* pTask, TaskState state );
        ...
        template< typename... ArgsT >
        Result CThreadPool::AddTask( ThreadUsages TaskUsages, cstr_t pDbgText, TaskFunction& Func, ArgsT&&... args )
        {
            Result ret = VKE_OK;
            Task   InternalTask;
            {
