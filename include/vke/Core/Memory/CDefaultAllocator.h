...
namespace VKE
{
    namespace Memory
    {
        struct IAllocator
        {
            vke_force_inline void* Allocate( const size_t& )
            {
                return nullptr;
            }

            vke_force_inline void* Allocate( const size_t&, const uint32_t& )
            {
                return nullptr;
            }

            vke_force_inline void Free( const size_t&, void** )
            {
            }

            /// Free array of elements
            vke_force_inline void Free( const size_t&, const uint32_t&, void** )
            {
            }
        };
        ...
        public:
            ...
            vke_force_inline void* Allocate( const size_t& size )
            {
                return VKE_MALLOC( size );
            }

            vke_force_inline void* Allocate( const size_t& elemSize, const uint32_t& count )
            {
                const auto size = elemSize * count;
                return VKE_MALLOC( size );
            }

            vke_force_inline void Free( const size_t& size, void** pPtrOut )
            {
                if( *pPtrOut && size > 0 )
                {
                    VKE_FREE( *pPtrOut );
            ...
            vke_force_inline void Free( const size_t& elementSize, const uint32_t& count, void** pPtrOut )
            {
                if( *pPtrOut && count > 0 )
                {
                    VKE_FREE( *pPtrOut );
