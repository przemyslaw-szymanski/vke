...
namespace VKE
{
    namespace Utils
    {
        ...
        public:
            ...
            CLogger& operator<<( const double& msg )
            {
                return Log( msg );
            }

            CLogger& operator<<( const float& msg )
            {
                return Log( msg );
            }

            template< typename _T_ >
            CLogger& operator<<( const _T_& msg )
            {
                return Log( msg );
            }
            ...
            void SetSeparator( const cstr_t pSeparator )
            {
            ...
            {
                return Platform::ThisThread::GetID();
            }
...
#define VKE_LOGGER_LOG2( _type, _msg )                                                                                 \
    VKE_CODE( VKE_LOGGER.Begin();                                                                                      \
              VKE_LOGGER << _type << "[" << VKE_LOG_TID << "]" << VKE_LOGGER_SEPARATOR << VKE_LOG_FUNC                 \
                         << VKE_LOGGER_SEPARATOR << VKE_LOG_LINE << VKE_LOGGER_SEPARATOR << _msg << "\n";              \
              VKE_LOGGER.Flush();                                                                                      \
              VKE_LOGGER.End(); )
