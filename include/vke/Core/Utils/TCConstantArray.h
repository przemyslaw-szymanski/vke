...
#define TC_CONSTANT_ARRAY_TEMPLATE_PARAMS DataType, DEFAULT_ELEMENT_COUNT, AllocatorType, Policy, Utils
        ...
        template< typename T, uint32_t DEFAULT_ELEMENT_COUNT = 32, class AllocatorType = Memory::CHeapAllocator,
                  class Policy = ConstantArrayDefaultPolicy, class Utils = ConstantArrayDefaultUtils >
        ...
        public:
            ...
            explicit TCConstantArray( uint32_t count ) : TCConstantArray()
            {
                auto res = Resize( count );
                assert( res );
            }

            TCConstantArray( uint32_t count, const DataTypeRef DefaultValue ) : TCConstantArray()
            {
            ...
            bool Append( const TCConstantArray& Other )
            {
                return Append( Other, 0, Other.GetCount() );
            }
            ...
            bool IsInConstArrayRange() const
            {
                return this->m_capacity < sizeof( m_aData );
            }

            TCConstantArray& operator=( const TCConstantArray& Other )
            {
                Other.Copy( this );
                return *this;
            }

            TCConstantArray& operator=( TCConstantArray&& Other )
            {
                Other.Move( this );
                return *this;
            }
        ...
        CONSTANT_ARRAY_TEMPLATE
        TCConstantArray< TC_CONSTANT_ARRAY_TEMPLATE_PARAMS >::TCConstantArray(
            std::initializer_list< DataType > List ) : TCConstantArray()
        {
        ...
        bool TCConstantArray< TC_CONSTANT_ARRAY_TEMPLATE_PARAMS >::Append( CountType begin, CountType end,
                                                                           const TCConstantArray& Other )
        {
            return Append( begin, end, Other.m_pCurrPtr );
        }

        CONSTANT_ARRAY_TEMPLATE
        bool TCConstantArray< TC_CONSTANT_ARRAY_TEMPLATE_PARAMS >::Append( CountType count, const DataTypePtr pData )
        {
