...
namespace VKE
{
    namespace Utils
    {
        ...
        {
            template< typename TCListT, uint32_t, class, class >
            friend class TCList;
        ...
        public:
            ...
            TCListIterator( const TCListIterator& Other ) : TCListIterator( Other.m_pCurr, Other.m_pData )
            {
            }
            ...
            DataTypeRef operator*() const
            {
                return m_pCurr->Data;
            }

            DataTypePtr operator->() const
            {
                return &m_pCurr->Data;
            }
        ...
        public:
            ...
            TCList( const TCList& Other ) : Base( Other )
            {
                _SetFirst( this->m_pCurrPtr );
                _SetIdxs( 0 );
            }

            TCList( TCList&& Other ) : Base( Other )
            {
                _SetFirst( this->m_pCurrPtr );
                _SetIdxs( 0 );
            }

            explicit TCList( CountType elemCount ) : Base( elemCount )
            {
                _SetFirst( this->m_pCurrPtr );
                _SetIdxs( 0 );
            }
            ...
            DataTypeRef Back()
            {
                return this->m_pCurrPtr[ m_lastAddedIdx ].Data;
            }
            ...
            Iterator Find( CountType idx )
            {
                return _Find< Iterator >( idx );
            }

            ConstIterator Find( CountType idx ) const
            {
                return _Find< ConstIterator >( idx );
            }

            Iterator Find( const DataTypeRef Data )
            {
                return _Find< Iterator >( Data );
            }

            ConstIterator Find( const DataTypeRef Data ) const
            {
                return _Find< ConstIterator >( Data );
            }
