...
namespace VKE
{
    namespace Utils
    {
        ...
        public:
            ...
            explicit TCRoundCounter( InputItr Begin, InputItr End ) : m_Begin( Begin ), m_End( End ), m_Curr( Begin )
            {
            }
        ...
        namespace Hash
        {
            ...
            // reads! Many thanks go to Yurii 'Hordi' Hordiienko, he lessened with 3 instructions the original 'Pippip',
            // thus:
            // #include <stdlib.h>
            // #include <stdint.h>
...
#define _PADr_KAZE( x, n ) ( ( ( x ) << ( n ) ) >> ( n ) )
            ...
            concept Hashable = requires( T Obj ) {
                { std::hash< T >{}( Obj ) } -> std::convertible_to< hash_t >;
            };

            template< typename T >
            concept HasCalcHash = requires( T Obj ) {
                { T::CalcHash( Obj ) } -> std::convertible_to< hash_t >;
            };
